...
Процедура УстановитьКартинкуСтатусаСтрокиДереваЭтапов(СтрокаДереваЭтапов)
	...
	Если ТипЗнч(СтрокаДереваЭтапов.Состояние) = Тип("ПеречислениеСсылка.ВажностьПроблемыУчета") Тогда
		...
		Если СтрокаДереваЭтапов.Состояние = Перечисления.ВажностьПроблемыУчета.Информация Тогда
			НомерКартинки = 5;
		ИначеЕсли СтрокаДереваЭтапов.Состояние = Перечисления.ВажностьПроблемыУчета.Предупреждение Тогда
			НомерКартинки = 2;
		ИначеЕсли СтрокаДереваЭтапов.Состояние = Перечисления.ВажностьПроблемыУчета.Ошибка Тогда
			НомерКартинки = 3;
		ИначеЕсли СтрокаДереваЭтапов.Состояние = Перечисления.ВажностьПроблемыУчета.ВажнаяИнформация Тогда
			НомерКартинки = 4;
		ИначеЕсли СтрокаДереваЭтапов.Состояние = Перечисления.ВажностьПроблемыУчета.ПолезныйСовет Тогда
			НомерКартинки = 9;
		КонецЕсли;
