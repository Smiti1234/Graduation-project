...
//@skip-warning
&НаКлиенте
Процедура Подключаемый_ПриИзмененииНастройкиАутентификации(Элемент, Текст, СтандартнаяОбработка)
...
//@skip-warning
&НаКлиенте
Процедура Подключаемый_НачалоВыбораНастройкиАутентификации(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ПолучитьУчастниковСБПЗапускЗаданияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Иначе
			...
			УчастникиСБП.Сортировать("ПорядокОтображения, Банк");
			...
			Если ЗначениеЗаполнено(ПлатежнаяСистема) Тогда
				...
				Если НастройкиБанка <> Неопределено Тогда
					ДобавитьНастройкиПодключения(
						...
						НастройкиБанка.АдресСтраницыЗаявки,
						НастройкиБанка.ПартнерАгентаСБП);
				Иначе
...
Процедура НастроитьПараметрыПодключения()
	...
	ИдентификаторУчастникаСБП = СервисИнтеграцииССБП.ИдентификаторУчастникаСБП(ПлатежнаяСистема, Ложь);
