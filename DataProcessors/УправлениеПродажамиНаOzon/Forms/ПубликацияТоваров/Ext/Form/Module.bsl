...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновленияДанныхРазрешено = ИнтеграцияСМаркетплейсомOzonВызовСервера.ОбновленияДанныхТорговойПлощадкиРазрешено(УчетнаяЗаписьМаркетплейса);
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.УправлениеПродажамиНаOzon.Форма.НастройкаОтображенияОстатков" Тогда
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			Для каждого КлючЗначение Из ПараметрыОтображенияОстатковЦенПеременная Цикл
				Если Тип(КлючЗначение.Значение) = Тип("Массив") Тогда
					ПараметрыОтображенияОстатковЦен.Вставить(КлючЗначение.Ключ, ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(КлючЗначение.Значение));
...
Процедура ТоварыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	// Остаток
	СопоставленныеСклады = ИнтеграцияСМаркетплейсомOzonСервер.ПолучитьСопоставленныеСклады(Настройки.ДополнительныеСвойства.УчетнаяЗаписьМаркетплейса);
	Если ПараметрыОтображенияОстатковЦен.ОстатокПоУчету.ОтображатьВТаблице
		...
		Запрос.УстановитьПараметр("Склады", СопоставленныеСклады);
			ТекстРеквизитаОстатка = 
				"СУММА(ЕСТЬNULL(ИсточникОстатков.ВНаличииОстаток - ИсточникОстатков.РезервироватьНаСкладеОстаток - ИсточникОстатков.РезервироватьПоМереПоступленияОстаток, 0) 
				|		/ ВЫБОР
			...
			|					ПО Склады.Ссылка В (&Склады))) КАК ИсточникОстатков
			...
			|			И (ИсточникОстатков.Склад В (&Склады))
			|			И (ИсточникОстатков.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка))
			|
			|СГРУППИРОВАТЬ ПО
			|	ТаблицаТоваров.Номенклатура,
			|	ТаблицаТоваров.Характеристика,
...
Процедура ТоварыСОшибкамиПриАктивизацииСтроки(Элемент)
	...
	Элементы.ТоварыСОшибкамиОтправитьНаМодерацию.Доступность = УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
...
Процедура ТоварыСОшибкамиПередУдалением(Элемент, Отказ)
	...
	ДлительнаяОперация = УдалитьТоварыНаСервере(УчетнаяЗаписьМаркетплейса, Элементы.ТоварыСОшибками.ВыделенныеСтроки, УникальныйИдентификатор);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаКлиенте
Процедура НастройкиВыгрузкиТоварногоКаталога(Команда)
...
Процедура ДобавитьПоОтбору(Команда)
	...
	ПараметрыФормы.Вставить("ВестиУчетСертификатовНоменклатуры", Ложь);
	ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", Новый ФиксированныйМассив(ИнтеграцияСМаркетплейсомOzonКлиентСервер.ОтборПоНоменклатуре()));
...
Процедура ИнициализироватьПараметрыОтображенияОстатковЦен()
	...
	Товары.УстановитьОграниченияИспользованияВГруппировке(СписокПолей);
	Товары.УстановитьОграниченияИспользованияВПорядке(СписокПолей);
	Товары.УстановитьОграниченияИспользованияВОтборе(СписокПолей);
...
Процедура УстановитьДоступностьЭлементовДляСтатуса()
	...
	Элементы.ОтправитьНаМодерацию.Доступность = УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
	Элементы.ПеренестиВАрхив.Доступность = УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено И Не СтатусВАрхиве И Не ПустаяСтрока(ТекущиеДанные.ИдентификаторТовара);
...
Процедура ОбновитьСтатистикуТоваровПродолжениеНаСервере()
	...
	Если Выборка.ЗагруженСОшибками <= 100 Тогда
		МассивСклонений = ПолучитьСклоненияСтрокиПоЧислу("позиция", Выборка.ЗагруженСОшибками, "", "ЧС=Количественное", "ПД=Именительный; ПЧ=Число", "ЧН=; ЧГ=");
	...
	Если Выборка.КПубликации <= 100 Тогда
		МассивСклонений = ПолучитьСклоненияСтрокиПоЧислу("позиция", Выборка.КПубликации, "", "ЧС=Количественное", "ПД=Именительный; ПЧ=Число", "ЧН=; ЧГ=");
	...
	Если Выборка.ИзмененыРеквизиты <= 100 Тогда
		МассивСклонений = ПолучитьСклоненияСтрокиПоЧислу("позиция", Выборка.ИзмененыРеквизиты, "", "ЧС=Количественное", "ПД=Именительный; ПЧ=Число", "ЧН=; ЧГ=");
...
Процедура ДобавитьПолучениеЦеныВТекстЗапроса(СтруктураПараметров)
	...
	СтруктураПараметров.РеквизитыЗапроса.Добавить("МАКСИМУМ(ТоварыСЦенами.Валюта)" + " КАК " + СтруктураПараметров.ИмяРеквизита + "Валюта");
...
Процедура УстановитьДоступностьВидимостьЭлементов()
	...
	Элементы.ОтправитьНаМодерацию.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
	Элементы.ВыгрузитьОстатки.Доступность = УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
	Элементы.ВыгрузитьЦены.Доступность = УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
	...
	Элементы.ПеренестиВАрхив.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
	...
	Элементы.ТоварыСОшибкамиОтправитьНаМодерацию.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана И ОбновленияДанныхРазрешено;
