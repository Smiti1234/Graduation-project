...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Документооборот_ДействиеСОбработкой" Тогда
		Если ТипЗнч(Параметр) = Тип("Структура")
			...
			Если Параметр.ТипОперации.Остановить Или Параметр.ТипОперации.Прервать Тогда
				Закрыть();
			Иначе
				ОбновитьФорму(Ложь, Ложь, Истина);
			КонецЕсли;
...
Процедура ВыполнитьДействиеЗадачиВариант1(Команда)
	...
	ДействиеНадЗадачей(ТипОперации, Новый Структура("РезультатВариантаНомер", "РезультатВарианта1"), Ложь);
...
Процедура ВыполнитьДействиеЗадачиВариант2(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.ВыполнитьДействиеЗадачи = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("РезультатВариантаНомер", "РезультатВарианта2"), Ложь);
...
Процедура ВыполнитьДействиеЗадачиВариант3(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.ВыполнитьДействиеЗадачи = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("РезультатВариантаНомер", "РезультатВарианта3"), Ложь);
...
Процедура УстановитьЖелтыйФлаг(Команда)
	...
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Yellow"));
...
Процедура УстановитьЗеленыйФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Green"));
...
Процедура УстановитьКрасныйФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Red"));
...
Процедура УстановитьЛиловыйФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Purple"));
...
Процедура УстановитьОранжевыйФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Orange"));
...
Процедура УстановитьСинийФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", "Blue"));
...
Процедура ОчиститьФлаг(Команда)
	...
	ТипОперации = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ТипОперацииНадЗадачей();
	ТипОперации.УстановитьФлаг = Истина;
	ДействиеНадЗадачей(ТипОперации, Новый Структура("Флаг", ""));
...
Процедура ОбработатьОтветВебСервисаНаПолучениеЗадачи(ЗадачаXDTO)
	...
	Если Не ЗначениеЗаполнено(ОсновнойПредметID) И Не ЗначениеЗаполнено(ОсновнойПредметТип) Тогда
		ОсновнойПредмет = ИнтеграцияС1СДокументооборот3.ОсновнойПредмет(ЗадачаXDTO);
...
&НаКлиенте
Процедура ДействиеНадЗадачей(ТипОперации, ПараметрыОперации = Неопределено, ТребуетсяОбновлениеДанных = Истина)
...
Процедура ДействиеНадЗадачейЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		РезультатДействия = ДействиеНадЗадачейНаСервереЗавершение(Результат.РезультатДлительнойОперации);
...
&НаСервере
Функция ДействиеНадЗадачейНаСервереЗавершение(Знач РезультатДействияСтрока)
