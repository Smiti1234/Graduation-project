...
Функция СвязанныеКоллекцииИРеквизиты()
	...
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Товары, "КодКатегории");
	
	Соответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Категории, ВложенноеСоответствие);
	
	ВложенноеСоответствие = Новый Соответствие();
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Продажи, "КодХарактеристики");
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы, "КодХарактеристики");
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Промо, "КодХарактеристики");
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж, "КодХарактеристики");
	...
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы, "КодКлиента");
	ВложенноеСоответствие.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж, "КодКлиента");
...
&НаСервере
Процедура ВыполнитьДействияПриПереключенииСтраницНаСервере()
...
Процедура ПолучитьНастройкиСервиса()
	...
	|ИдентификаторПриложения, ТокенПриложения, ЗаявкаНаПодключениеОтправлена, ИзмененыНастройкиИнтернетПоддержки,
...
Процедура СформироватьДеревоВыгрузкиМетаданных()
	...
	Для Каждого Коллекция Из КоллекцииПоПорядку Цикл
		...
		Если ОписаниеКоллекции.ИмяВИсточнике = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Продажи
			Или ОписаниеКоллекции.ИмяВИсточнике = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы
...
Процедура ЗаполнитьСводкуНастроек()
	...
	ПодключениеПроизведеноВыполненаВыгрузка = ПодключениеПроизведено И ЗначениеЗаполнено(ДанныеПоПродажам.ДатаНачалаПоследнейВыгрузки);
	...
	ШаблонКоллекцииКВыгрузке = НСтр("ru='<u>Коллекции к выгрузке:</u>
	|%1'");
	...
	ШаблонЗаголовокАнализДанных = НСтр("ru='<u>Анализ имеющихся и выгруженных данных</u>'");
	...
	Для Каждого КлючЗначение Из Настройки.Коллекции Цикл
		Если КлючЗначение.Значение.Выгружать Тогда
			ПредставлениеКоллекций.Добавить(КлючЗначение.Значение.Представление);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.КонтрольныеПланыПродаж.Видимость = Истина;
...
Процедура УказательНаСтраницыОбработкаНавигационнойСсылкиНаСервере(НавигационнаяСсылкаФорматированнойСтроки)
	...
	ВыполнитьДействияПриПереключенииСтраницНаСервере();
...
Процедура СохранитьСоставПередаваемыхДанных()
	...
	Для Каждого УровеньКоллекций Из НастройкиВыгрузкиМетаданных.ПолучитьЭлементы() Цикл
		...
		Для Каждого УровеньРеквизитов Из УровеньКоллекций.ПолучитьЭлементы() Цикл
			...
			КоллекцияВыгружалась = РегистрыСведений.КоллекцииСервисаПрогнозированияПродаж.КоллекцияВыгружалась(УровеньКоллекций.ИмяВИсточнике);
			...
			Если КоллекцияВыгружалась
				...
				СводкаПоКоллекции= РегистрыСведений.КоллекцииСервисаПрогнозированияПродаж.ПолучитьИнформациюКоллекции(УровеньКоллекций.ИмяВИсточнике);
				СводкаПоКоллекции.ИзмененФормат = Истина;
				РегистрыСведений.КоллекцииСервисаПрогнозированияПродаж.ЗаписатьИнформациюКоллекции(УровеньКоллекций.ИмяВИсточнике, СводкаПоКоллекции);
...
Процедура ЗаписатьНастройки()
	...
	КоллекцияТовары = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Товары;
...
Процедура НачатьПроверкуДанныхНаСервере()
	...
	Для Каждого КлючЗначение Из НастройкиСервиса.Коллекции Цикл
		...
		Если Сводка.Свойство("ПричинаОшибки") Тогда
			Если Сводка.ПричинаОшибки = "НеУказанКалендарьПредприятия" Тогда
				ТекстыОшибок.Добавить(ТекстОшибкиНеУказанКалендарьПредприятия());
			ИначеЕсли Сводка.ПричинаОшибки = "НеУказанСценарийПрогнозированияКонтрольныхПланов" Тогда
				ТекстыОшибок.Добавить(ТекстОшибкиНеУказанСценарийПрогнозированияКонтрольныхПланов());
			КонецЕсли;
			ЕстьПроблемы = Истина;
		Иначе
