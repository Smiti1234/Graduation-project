...
Процедура СоздатьДокументыОплаты(ПараметрыПроцедуры, АдресХранилищаРезультата) Экспорт
	...
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", ПараметрыПроцедуры.Организация);
	ДанныеЗаполнения.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
Функция ДокументыПриобретения(ИсходныеДанные, ДанныеЗаполнения)
	...
	Для Каждого СтрокаДанных Из ИсходныеДанные.ТаблицаДанных Цикл
		...
		Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
			...
			Если Не ПустаяСтрока(СтрокаДанных.ЧекСамозанятого) Тогда
				
				ИмяФайла = СтрШаблон(НСтр("ru='№%1'"), СтрокаДанных.НомерЧека);
				...
				АдресФайла = ПоместитьВоВременноеХранилище(СтрокаДанных.ЧекСамозанятого);
				РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресФайла);
				
			ИначеЕсли Не ПустаяСтрока(СтрокаДанных.СсылкаНаЧек) Тогда
				ДенежныеСредстваСерверЛокализация.ДобавитьЧекСамозанятогоИзИнтернета(
					СтрокаДанных.СсылкаНаЧек, ДокументОбъект.Ссылка);
			КонецЕсли;
...
Процедура ЗаполнитьТабличнуюЧастьТовары(ДокументОбъект, ИмяТабличнойЧасти, ДанныеЗаполнения, ОстаткиПоЗаказам)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "ПоступлениеПоЗаказам") Тогда
		...
		ПараметрыПересчета.Вставить("РеализацияПоступлениеПоЗаказу", ДокументОбъект.ПоступлениеПоЗаказам);
		ПараметрыПересчета.Вставить("ТребуетсяЗалогЗаТару", ДокументОбъект.ТребуетсяЗалогЗаТару);
		СтруктураДействий.Вставить("ПересчитатьСуммуСверхЗаказа", ПараметрыПересчета);
...
Процедура СопоставитьЗагружаемыеДанныеНаОплату(
	...
	Пока ВыборкаИзФайла.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.БанковскийСчетКонтрагента) Тогда
			ОтборДоговоровПоКонтрагенту.Вставить("БанковскийСчет", НоваяСтрока.БанковскийСчетКонтрагента);
		Иначе
			ОтборДоговоровПоКонтрагенту.Вставить("БанковскийСчет", Справочники.БанковскиеСчетаКонтрагентов.ПустаяСсылка());
		КонецЕсли;
