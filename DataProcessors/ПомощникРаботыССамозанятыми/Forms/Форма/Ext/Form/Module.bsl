...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.ДокументыПриобретенияЗагрузитьЧекСамозанятого.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
...
Процедура ДобавитьСтрокиВДокументыОплаты(Данные)
	...
	Для Каждого ОстатокОплаты Из ОстаткиОплатыПоСамозанятому Цикл
		...
		Если ОстатокОплаты.КОплате > 0 Тогда
			
			НоваяСтрока = СтрокаТаблицыОплаты(ОстатокОплаты);
			
			НоваяСтрока.Сумма = ОстатокОплаты.КОплате;
...
Процедура ЗаполнитьСписокПоПрошломуМесяцуНаСервере()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ДокументСписания.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаСамозанятому)
...
Процедура ОткрытьФормуЧекаСамозанятого(Источник)
	...
	ОткрытьФорму("Обработка.ПомощникРаботыССамозанятыми.Форма.ФормаЧекаСамозанятого", 
		ПараметрыФормы, , , , ,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Если ЗначениеЗаполнено(ТекущаяСтрокаТаблицы.Договор) Тогда
		...
		ТекущаяСтрокаТаблицы.СтатьяДвиженияДенежныхСредств = 
			ДенежныеСредстваСервер.СтатьяДвиженияДенежныхСредствОбъектаРасчетов(
				ТекущаяСтрокаТаблицы.ОбъектРасчетов, Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
Функция ОстаткиПоОбъектамРасчетов(ДебиторскаяЗадолженность)
	...
	Реквизиты.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
Процедура ОграничитьОстаткиЗадолженностиПоставщика(ОстаткиВзаиморасчетов)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СписаниеБезналичныхДенежныхСредств.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаСамозанятому)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
