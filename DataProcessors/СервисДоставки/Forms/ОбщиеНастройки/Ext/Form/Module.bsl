...
Процедура СоответствиеСкладовСкладПриИзмененииНаСервере()
	...
	ТекущиеДанные = СоответствиеСкладов.НайтиПоИдентификатору(Элементы.СоответствиеСкладов.ТекущаяСтрока);
	ТекущиеДанные.СкладАдрес = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(ТекущиеДанные.Склад, Справочники.ВидыКонтактнойИнформации.АдресСклада);
...
Процедура ОбновитьЗаголовокСоответствиеПунктовВыдачи()
	...
	КоличествоЗаполненных = КоличествоЗаписей - СоответствиеСкладов.НайтиСтроки(Новый Структура("Склад", ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"))).Количество();
