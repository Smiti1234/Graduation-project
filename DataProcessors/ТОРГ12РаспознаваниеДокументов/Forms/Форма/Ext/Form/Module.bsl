...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.АктОбОказанииУслуг Тогда
		...
		Если Не ПравоДоступа("Редактирование", Метаданные.Документы.АктВыполненныхРабот) Тогда
...
Функция ПроверитьВозможностьСозданияНоменклатуры()
	...
	Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Номенклатура.ПустаяСсылка()) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура НастроитьИзменяемыйЭлемент(РеквизитШапки, ИзменяемыйЭлемент, ДанныеДляВыбораРеквизитов)
	...
	Если НЕ РеквизитШапки.ИмяРеквизита = "Склад" И НЕ РеквизитШапки.ИмяРеквизита = "Договор" И НЕ РеквизитШапки.ИмяРеквизита = "Подразделение"
		...
		Если РеквизитШапки.ИмяРеквизита = "Партнер" Тогда
			Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Партнеры.ПустаяСсылка()) Тогда
				ИзменяемыйЭлемент.СписокВыбора.Удалить(0);
			КонецЕсли;
		КонецЕсли;	
		Если РеквизитШапки.ИмяРеквизита = "Продавец" Тогда
			Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Контрагенты.ПустаяСсылка()) Тогда
				ИзменяемыйЭлемент.СписокВыбора.Удалить(0);
			КонецЕсли;
		КонецЕсли;     
		Если РеквизитШапки.ИмяРеквизита = "Покупатель" Тогда
			Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Контрагенты.ПустаяСсылка()) Тогда
				ИзменяемыйЭлемент.СписокВыбора.Удалить(0);
			КонецЕсли;
		КонецЕсли;    
		Если РеквизитШапки.ИмяРеквизита = "Грузоотправитель" И ИзменяемыйЭлемент.СписокВыбора.Количество() Тогда
            Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Контрагенты.ПустаяСсылка()) Тогда
				ИзменяемыйЭлемент.СписокВыбора.Удалить(0);
			КонецЕсли;
		ИначеЕсли РеквизитШапки.ИмяРеквизита = "Грузополучатель" И ИзменяемыйЭлемент.СписокВыбора.Количество() Тогда
            Если Не УправлениеДоступом.ИзменениеРазрешено(Справочники.Контрагенты.ПустаяСсылка()) Тогда
				ИзменяемыйЭлемент.СписокВыбора.Удалить(0);
			КонецЕсли;
		КонецЕсли;
...
Процедура ИзменитьОтображениеПоВидуДокумента()
	...
	Если Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда 
		...
		Если ВидДокумента = "ПТИУ" И Не ПравоДоступа("Редактирование", Метаданные.Документы.ПриобретениеТоваровУслуг) Тогда
			Элементы.ФормаСоздатьПоступлениеТовары.Видимость = Ложь;
			Элементы.ФормаСоздатьПоступлениеТоварыСчетФактура.Видимость = Ложь;   
			ВидДокумента = "ПУИПА"   
		КонецЕсли;
		
		Если ВидДокумента = "ПУИПА" И Не ПравоДоступа("Редактирование", Метаданные.Документы.ПриобретениеУслугПрочихАктивов) Тогда
	...
	Иначе       
		...
		Если ВидДокумента = "РТИУ" И Не ПравоДоступа("Редактирование", Метаданные.Документы.РеализацияТоваровУслуг) Тогда
			Элементы.СоздатьРеализацияТовары.Видимость = Ложь;
			Элементы.СоздатьРеализацияТоварыСчетФактура.Видимость = Ложь;   
			ВидДокумента = "РУИПА"   
		КонецЕсли;
		
		Если ВидДокумента = "РУИПА" И Не ПравоДоступа("Редактирование", Метаданные.Документы.РеализацияУслугПрочихАктивов) Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	//Склад для входящих/исходящих с услугами/работами
	Элемент = УсловноеОформление.Элементы.Добавить();
