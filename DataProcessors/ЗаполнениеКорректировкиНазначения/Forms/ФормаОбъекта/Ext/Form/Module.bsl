...
Процедура ПроверитьСоставТоварныхМест(ТаблицаКонтроляТоварныхМест, Ошибки)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаДанных", ТаблицаКонтроляТоварныхМест);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ТаблицаДанных.УпаковкаТовара КАК УпаковкаТовара,
	...
	|	ТаблицаДанных.Склад КАК Склад,
	|	ТаблицаДанных.УпаковкаТовара КАК УпаковкаТовара,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаУпаковокТоварныеМеста.Ссылка) КАК КоличествоТоварныхМестПлан,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Упаковка) КАК КоличествоТоварныхМестФакт,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаУпаковокТоварныеМеста.Ссылка) <> КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Упаковка) КАК
	|		ЕстьОшибкаСостава,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Количество) > 1 КАК ЕстьОшибкаКратности
	|ИЗ
	|	ТаблицаДанных КАК ТаблицаДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ТаблицаУпаковок
	|		ПО ТаблицаДанных.Упаковка = ТаблицаУпаковок.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ТаблицаУпаковокТоварныеМеста
	|		ПО ТаблицаУпаковок.Владелец = ТаблицаУпаковокТоварныеМеста.Владелец
	|		И ТаблицаУпаковок.ЕдиницаИзмерения = ТаблицаУпаковокТоварныеМеста.ЕдиницаИзмерения
	|		И ТаблицаУпаковок.ТипУпаковки = ТаблицаУпаковокТоварныеМеста.ТипУпаковки
	|		И НЕ ТаблицаУпаковокТоварныеМеста.ПометкаУдаления
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаДанных.Склад,
	|	ТаблицаДанных.УпаковкаТовара
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаУпаковокТоварныеМеста.Ссылка) <> КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Упаковка)
	|	ИЛИ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Количество) > 1";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.ЕстьОшибкаСостава Тогда
			
			ПредставлениеРеквизитовНоменклатуры = ПолучитьПредставлениеРеквизитовНоменклатуры(Выборка);
			
			Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Из выбранных товарных мест нельзя собрать целое количество <%1> товара ""%2""%3: выбраны не все виды товарных мест (%4 из %5).'"),
				Выборка.УпаковкаТовара,
				Выборка.Номенклатура,
				ПредставлениеРеквизитовНоменклатуры,
				Выборка.КоличествоТоварныхМестФакт,
				Выборка.КоличествоТоварныхМестПлан));
			
		ИначеЕсли Выборка.ЕстьОшибкаКратности Тогда
			
			ПредставлениеРеквизитовНоменклатуры = ПолучитьПредставлениеРеквизитовНоменклатуры(Выборка);
			
			Ошибки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Из выбранных товарных мест нельзя собрать целое количество <%1> товара ""%2""%3: не выполнено условие кратности для выбранных товарных мест.'"),
				Выборка.УпаковкаТовара,
				Выборка.Номенклатура,
				ПредставлениеРеквизитовНоменклатуры));
...
&НаСервереБезКонтекста
Функция ПолучитьПредставлениеРеквизитовНоменклатуры(Выборка)
	
	МассивПредставлений	= Новый Массив;
	
	Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
		МассивПредставлений.Добавить(СтрШаблон(НСтр("ru = 'характеристика %1'"), Выборка.Характеристика));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Выборка.Серия) Тогда
		МассивПредставлений.Добавить(СтрШаблон(НСтр("ru = 'серия %1'"), Выборка.Серия));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Выборка.Назначение) Тогда
		МассивПредставлений.Добавить(СтрШаблон(НСтр("ru = 'назначение %1'"), Выборка.Назначение));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Выборка.Склад) Тогда
		МассивПредставлений.Добавить(СтрШаблон(НСтр("ru = 'склад %1'"), Выборка.Склад));
	КонецЕсли;
		
	ПредставлениеРеквизитов = СтрСоединить(МассивПредставлений, ", ");
	Если СтрДлина(ПредставлениеРеквизитов) > 0 Тогда
		ПредставлениеРеквизитов = СтрШаблон(" (%1)", ПредставлениеРеквизитов);
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ДобавитьТоварноеМесто(Ветвь, РодительскаяВетвь, ТаблицаКонтроляТоварныхМест, Количество)
	
	ТипУпаковки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ветвь.Упаковка, "ТипУпаковки");
	Если ТипУпаковки = ПредопределенноеЗначение("Перечисление.ТипыУпаковокНоменклатуры.ТоварноеМесто") Тогда
		
		НоваяСтрока = ТаблицаКонтроляТоварныхМест.Добавить();
		НоваяСтрока.Номенклатура = РодительскаяВетвь.Номенклатура;
		НоваяСтрока.Характеристика = РодительскаяВетвь.Характеристика;
		НоваяСтрока.Серия = Ветвь.ПолучитьРодителя().Серия;
		НоваяСтрока.Назначение = РодительскаяВетвь.Назначение;
		НоваяСтрока.Склад = РодительскаяВетвь.Склад;
		НоваяСтрока.УпаковкаТовара = РодительскаяВетвь.Упаковка;
		НоваяСтрока.Упаковка = Ветвь.Упаковка;
		НоваяСтрока.Количество = Ветвь.Количество;
...
Функция МожноПеренестиДанныеДереваВТаблицу(Ошибка)
	...
	ТаблицаКонтроляТоварныхМест = Новый ТаблицаЗначений;
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Серия", Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Назначение", Новый ОписаниеТипов("СправочникСсылка.Назначения"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("УпаковкаТовара", Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Упаковка", Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
	ТаблицаКонтроляТоварныхМест.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	...
	Для Каждого Корень Из ДеревоПомещенияЯчейки.ПолучитьЭлементы() Цикл
		...
		Для Каждого Ветвь Из Ветви Цикл
			...
			Если Ветвь.Отметка Тогда
				...
				ДобавитьТоварноеМесто(Ветвь, Корень, ТаблицаКонтроляТоварныхМест, Количество);
				
			Иначе
				
				КоличествоПоЯчейке = 0;
				
				Для Каждого ВетвьЯчейки Из Ветвь.ПолучитьЭлементы() Цикл
					
					Если ВетвьЯчейки.Отметка Тогда
						
						ДобавитьТоварноеМесто(ВетвьЯчейки, Корень, ТаблицаКонтроляТоварныхМест, Количество);
						КоличествоПоЯчейке = ВетвьЯчейки.Количество;	
						
					КонецЕсли;
					
				КонецЦикла;
				
				Количество = Количество + КоличествоПоЯчейке;
		...
		Если Количество <> Корень.Количество Тогда
			...
			Если ЗначениеЗаполнено(Корень.Характеристика) Тогда
				...
				СтрокаТовар = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1, %2",
					Корень.Номенклатура,
					Корень.Характеристика);
			...
			НоваяОшибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По товару %1 на складе %2 требуется указать %3 %4, а указано %5 %4.'"),
				СтрокаТовар,
				Корень.Склад,
				Корень.Количество,
				Корень.Упаковка,
	...
	ПроверитьСоставТоварныхМест(ТаблицаКонтроляТоварныхМест, Ошибки);
