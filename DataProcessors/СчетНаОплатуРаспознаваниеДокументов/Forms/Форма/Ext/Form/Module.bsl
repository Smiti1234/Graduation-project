...
Функция ПолучитьПараметрыОткрытияФормы(Знач ВидОперации, Знач ТипДокументаСтрокой)
	...
	//Записать();
	//ПараметрыЗаполнения = РаспознаваниеДокументовКомплектыВызовСервера.ПараметрыЗаполненияТиповойНаОсновеСчетНаОплату(
	//	Объект,
	//	ТипДокументаСтрокой
	//);
	//
	...
	Для Каждого ЭтотСвязанный Из ВсеСвязанные Цикл
		...
		Для Каждого ЭтотТип Из ПодходящиеТипы Цикл
			...
			Если КомплектныеСтроки.Количество() <> 0 Тогда
				//Если (ТипСвязанного = Тип("ДокументСсылка.ЗаказКлиента") И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов")) 
				//	Или (ТипСвязанного = Тип("ДокументСсылка.ЗаказПоставщику") И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыПоставщикам")) Тогда
				...
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.ЗаказКлиента") 
					//И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов") 
					Тогда
					СозданныеДокументы.ДокументИсходящий.Ссылка = ЭтотСвязанный;
				ИначеЕсли ТипСвязанного = Тип("ДокументСсылка.ЗаказПоставщику")
