...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗачетОплатПоДатеПлатежа.Видимость = НаборКонстант.ЗачетОплатПоДатеПлатежа;
...
Процедура НоваяАрхитектураРасчетовОтключениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			НаборКонстант.ЗачетОплатПоДатеПлатежа = 0;
...
Процедура ПорядокЗачетаДокументовПриИзменении(Элемент)
	...
	Ответ = Неопределено;
	ТекстВопроса = НСтр("ru = 'Переключение режима зачета документов может занять продолжительное время.
		|Продолжить?'");
...
Процедура ПорядокЗачетаДокументовПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Результат = ЗапуститьРаспределениеПлановыхРасчетовФоновымЗаданием();
		Если Результат.Статус = "Выполняется" Тогда
			ФормаДлительнойОперации = ДлительныеОперацииКлиент.ОткрытьФормуДлительнойОперации(ЭтаФорма, ИдентификаторЗадания);
			ИдентификаторЗадания = Результат.ИдентификаторЗадания;
			ФормаДлительнойОперации.ИдентификаторЗадания = ИдентификаторЗадания;
			ДлительныеОперацииКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПараметрыОбработчикаОжидания.КоэффициентУвеличенияИнтервала = 1.2; // Уменьшим шаг увеличения времени опроса выполнения задания
			ПодключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеЗаданияРаспределенияПлановыхРасчетов", 1, Истина);
		КонецЕсли;
		
	Иначе
		ЗачетОплатПоДатеПлатежа = ?(ЗачетОплатПоДатеПлатежа = 1,0,1);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПроверитьВыполнениеЗаданияРаспределенияПлановыхРасчетов()
	Попытка
		...
		Если ФормаДлительнойОперации.Открыта()
			...
			Если НЕ ЗаданиеВыполнено(ИдентификаторЗадания) Тогда 
				ДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
				ПодключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеЗаданияРаспределенияПлановыхРасчетов", ПараметрыОбработчикаОжидания.ТекущийИнтервал, Истина);
			Иначе
				ДлительныеОперацииКлиент.ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			КонецЕсли;
		Иначе
...
&НаСервере
Функция ЗапуститьРаспределениеПлановыхРасчетовФоновымЗаданием()
	
	ПараметрыЭкспортнойПроцедуры = Новый Структура;
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =НСтр("ru = 'Выполняется перезаполнение плановых взаиморасчетов'");
	
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"ОперативныеВзаиморасчетыСервер.ЗаполнитьПлановыеРасчеты",
		ПараметрыЭкспортнойПроцедуры,
		ПараметрыВыполнения);
