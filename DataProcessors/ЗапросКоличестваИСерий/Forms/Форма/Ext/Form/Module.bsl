...
&НаСервере
Функция ДобавитьПодобранныеТовары(ПодобранныеТовары, СтрокиТовары, Упаковка, Цена, КоэффициентУпаковки, ТипНоменклатуры = Неопределено, ХарактеристикиИспользуютсяПоКомлектующей = Ложь)
	
	СтрокаДокументаДобавляестяВпервые = Истина;
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		Если ЗначениеЗаполнено(Объект.ВариантКомплектацииНоменклатуры) Тогда
			ПараметрыТовара.ХарактеристикиИспользуются = ХарактеристикиИспользуютсяПоКомлектующей;
		Иначе
			ПараметрыТовара.ХарактеристикиИспользуются = ХарактеристикиИспользуются;
		КонецЕсли;
		...
		Для Каждого СтрокаСерии Из СтрокиСерий Цикл
			...
			Если СтрокаДокументаДобавляестяВпервые И СтрокаТовары.ЭтоСтрокаДокумента Тогда
				СтрокаДокументаДобавляестяВпервые = Ложь;
				СтрокаССерией.ЭтоНоваяСтрока = Ложь;
			КонецЕсли;
		...
		Если Не ОкруглениеКорректно
				И ЗначениеЗаполнено(Упаковка)
				И Окр(КоличествоСерий, 0, РежимОкругления.Окр15как20) = Окр(СтрокаТовары.Подобрано * КоэффициентУпаковки, 0, РежимОкругления.Окр15как20) Тогда
				...
				Запрос.Текст =
					...
					|ГДЕ
					|	ВЫРАЗИТЬ(&Номенклатура КАК Справочник.Номенклатура).ЕдиницаИзмерения.ТипИзмеряемойВеличины
	...
	Возврат СтрокаДокументаДобавляестяВпервые;
...
Функция ПодобранныеТовары()
	...
	Если Объект.ВариантКомплектацииНоменклатуры.Пустая() Тогда
		КоэффициентУпаковки = КоэффициентУпаковки(Объект.Упаковка, Объект.Номенклатура);
		СтрокаДокументаДобавляестяВпервые = ДобавитьПодобранныеТовары(
			ПодобранныеТовары, Объект.Товары, Объект.Упаковка, Объект.Цена, КоэффициентУпаковки);
		...
		Если ОсталосьПодобрать > 0 Тогда
			...
			Если Объект.ИндексСтрокиДокумента > -1 Тогда
				...
				ПараметрыТовара.ЭтоНоваяСтрока = Не СтрокаДокументаДобавляестяВпервые;
				ПараметрыТовара.ХарактеристикиИспользуются = ХарактеристикиИспользуются;
				ПодобранныеТовары.Добавить(ПараметрыТовара);
	...
	Иначе
		...
		ВсеКомплектующие = Объект.СоставНабора.Выгрузить(, "Номенклатура").ВыгрузитьКолонку("Номенклатура");
		ТипыКомплектующих = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВсеКомплектующие, "ТипНоменклатуры");
		ХарактеристикиИспользуютсяПоКомлектующим = ЗаполнитьИспользованиеХарактеристик(ВсеКомплектующие);
		
		Для Каждого ПозицияНабора Из Объект.СоставНабора Цикл
			
			ХарактеристикиИспользуютсяПоКомлектующей = ХарактеристикиИспользуютсяПоКомлектующим.Получить(ПозицияНабора.Номенклатура);
			...
			ДобавитьПодобранныеТовары(
				...
				ПозицияНабора.Цена,
				КоэффициентУпаковки,
				ТипыКомплектующих.Получить(ПозицияНабора.Номенклатура),
				ХарактеристикиИспользуютсяПоКомлектующей);
			...
			Если Подобрано > 0 Тогда
				...
				ПараметрыТовара.ТипНоменклатуры = ТипыКомплектующих.Получить(ПозицияНабора.Номенклатура);
				Если Склады.Количество() = 1 тогда
					ПараметрыТовара.Склад = Склады[0].Значение;
				Иначе
					ПараметрыТовара.Склад = Справочники.Склады.ПустаяСсылка();
...
&НаСервере
Функция ЗаполнитьИспользованиеХарактеристик(ВсеКомплектующие)
	
	ХарактеристикиИспользуютсяПоКомлектующим = Новый Соответствие;
	...
	Пока Выборка.Следующий() Цикл
		ХарактеристикиИспользуютсяПоКомлектующим.Вставить(Выборка.Номенклатура, Выборка.ХарактеристикиИспользуются);
	КонецЦикла;
	
	Возврат ХарактеристикиИспользуютсяПоКомлектующим;
