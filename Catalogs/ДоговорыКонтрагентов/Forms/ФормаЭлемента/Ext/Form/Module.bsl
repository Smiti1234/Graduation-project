...
Процедура ПриОткрытии(Отказ)
	...
	Если ЕстьПравоДобавленияФайла Тогда
		ОписаниеЗавершения = Новый ОписаниеОповещения("ДоступнаКомандаСканироватьЗавершение", ЭтотОбъект);		
		РаботаСФайламиКлиент.ДоступнаКомандаСканировать(ОписаниеЗавершения);
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	Если Не ЗавершениеРаботы
		И ПроверкаПриСозданииДоговора И ЗначениеЗаполнено(Объект.Ссылка) Тогда
...
Процедура ДоговорСКомиссионеромПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ДоговорСКомиссионером) Тогда
		...
		Если Объект.ВалютаВзаиморасчетов = ДоговорСКомиссионером.ВалютаВзаиморасчетов Тогда
...
Процедура ПорядокРасчетовПриИзмененииСервер()
	...
	Если Объект.ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки
		И (Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
			Или Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам) Тогда
		Объект.ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.Переменный;
	КонецЕсли;
...
Процедура НастроитьПараметрыВыбораСтатьиДвиженияДенежныхСредств()
	...
	//++ Локализация
	ТипыДоговоровСамозанятый = Новый Массив;
	ТипыДоговоровСамозанятый.Добавить(Перечисления.ТипыДоговоров.СПоставщиком);
	ТипыДоговоровСамозанятый.Добавить(Перечисления.ТипыДоговоров.СКомитентом);
	//-- Локализация
	...
	Иначе
		...
		//++ Локализация
		Если ИспользоватьВыплатыСамозанятым
			И ТипыДоговоровСамозанятый.Найти(Объект.ТипДоговора) <> Неопределено Тогда
			СписокХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
		КонецЕсли;
		//-- Локализация
...
Процедура УстановитьВидимостьНоваяСхемаКомиссии()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, 
		"ЛокализацияГруппаВариантВыбытияМаркируемойПродукции, ГруппаНалогообложениеНДС, РазрешенаРаботаСДочернимиПартнерами",
...
&НаКлиенте
Процедура ДоступнаКомандаСканироватьЗавершение(Результат, Параметры = Неопределено) Экспорт
	
	Элементы.ДобавитьФайлСоСканера.Видимость = Результат;
	Элементы.СписокКонтекстноеМенюДобавитьФайлСоСканера.Видимость = Результат;
...
Процедура НастроитьДинамическийСписокФайлов()
	...
	// ЭлектронноеВзаимодействие.ОбменСКонтрагентами
	|	ВЫБОР
	|		КОГДА Файлы.Расширение = &Расширение
...
Процедура УстановитьДоступностьФайловыхКоманд(Результат = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт
	...
	ИначеЕсли ТекущиеДанные <> Неопределено 
		...
		// ЭлектронноеВзаимодействие.ОбменСКонтрагентами
		Если ИнтерфейсДокументовЭДОУТКлиент.ФормаДоговорногоДокументаОткрыта(Объект.Ссылка) Тогда
...
Функция ПолучитьИменаКомандИзмененияОбъектов()
	...
	ИменаКоманд.Добавить("ЗакончитьРедактированиеФайла");
	ИменаКоманд.Добавить("ЗанятьФайл");
	ИменаКоманд.Добавить("ОсвободитьФайл");
	ИменаКоманд.Добавить("РедактироватьФайл");
	ИменаКоманд.Добавить("УстановитьПометкуУдаленияФайла");
	ИменаКоманд.Добавить("УдалитьФайл");
	ИменаКоманд.Добавить("КонтекстноеМенюУстановитьПометкуУдаления");
	
	ИменаКоманд.Добавить("ПодписатьФайл");
	ИменаКоманд.Добавить("ДобавитьЭПИзФайла");
	ИменаКоманд.Добавить("СохранитьФайлВместеСЭП");
	
	ИменаКоманд.Добавить("ЗашифроватьФайл");
	ИменаКоманд.Добавить("РасшифроватьФайл");
	
	ИменаКоманд.Добавить("ПечатьФайла");
	ИменаКоманд.Добавить("ПечатьСоШтампомФайла");
	
	ИменаКоманд.Добавить("ОтправитьФайл");
	
	ИменаКоманд.Добавить("ОбновитьИзФайлаНаДиске");
	
	// Команды независимые от состояния объектов.
	ИменаКоманд.Добавить("ДобавитьФайлСДиска");
	ИменаКоманд.Добавить("ДобавитьИзФайлаНаДиске");
	ИменаКоманд.Добавить("ДобавитьФайлПоШаблону"); 
	ИменаКоманд.Добавить("ДобавитьФайлПоШаблонуДоговора");
	ИменаКоманд.Добавить("ДобавитьФайлСоСканера");
	ИменаКоманд.Добавить("ОткрытьСвойстваФайла");
	ИменаКоманд.Добавить("СкопироватьФайл");
	ИменаКоманд.Добавить("ЗагрузитьФайлы");
	ИменаКоманд.Добавить("ЗагрузитьПапкуФайлов");
	
	ИменаКоманд.Добавить("СоздатьГруппуФайлов");
	ИменаКоманд.Добавить("ПереместитьВГруппуФайлов");
