...
Процедура ВыбратьПлатежиВыполнить()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОстатковРасчетов Цикл
		Если Элементы.ТаблицаОстатковРасчетов.ПроверитьСтроку(СтрокаТаблицы.ПолучитьИдентификатор()) Тогда
...
Процедура РассчитатьСуммуПлатежей()
	...
	Если НЕ ЗначениеЗаполнено(ВалютаВзаиморасчетов) Тогда
		...
		ИначеЕсли ОднаВалютаВзаиморасчетов Тогда
			Если Элементы.ТаблицаОстатковРасчетов.ТекущиеДанные <> Неопределено Тогда
			...
			Иначе
				...
				Если ВыбранныеСтроки.Количество() > 0 Тогда
					ТекущаяСтрока = ВыбранныеСтроки[ВыбранныеСтроки.ВГраница()];
					ОтборСтрок = Новый ФиксированнаяСтруктура("ВалютаВзаиморасчетов", ТекущаяСтрока.ВалютаВзаиморасчетов);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Иначе
