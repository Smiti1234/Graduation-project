...
//++ Локализация
	...
	Если ИнтеграцияСПлатежнымиСистемами.ИнтеграцияДоступна() Тогда
		...
		Для Каждого ДействующийДоговор Из ДействующиеДоговора Цикл
			Попытка
				ПараметрыНастройкиПодключения = ИнтеграцияСПлатежнымиСистемами.НастройкиТорговойТочки(ДействующийДоговор.Интеграция);
			Исключение
			...
			НоваяСтрокаНастроек.КассовыеСсылки = ПараметрыНастройкиПодключения.КассовыеСсылки; 
			НоваяСтрокаНастроек.ПодключениеКассовойСсылки = ПараметрыНастройкиПодключения.ПодключениеКассовойСсылки;
		КонецЦикла;
	КонецЕсли;
...
Функция ПечатьКассовойСсылкиПоШаблонуНаСервере(НастройкаПечати)
	...
	Возврат ИнтеграцияСПлатежнымиСистемами.СформироватьКарточкуКассовойСсылки(ДанныеКассы, ПараметрыПечати);
