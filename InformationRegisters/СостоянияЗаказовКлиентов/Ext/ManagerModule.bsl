...
Функция ТекстЗапросаДляРасчетаСостоянийОбеспеченияЗаказа()
	...
	ТекстЗапроса = "
		...
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ) КАК ЕстьТребующиеРаспределенияЗапасов,
		|	
...
Функция ТекстЗапросаСостояниеВзаиморасчетов()
	...
	ТекстЗапроса = "
		...
		|			ТОГДА 0
		|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(Расчеты.СуммаОплат) * 100 / МАКСИМУМ(Расчеты.СуммаПервоначальныхВзаиморасчетов) КАК ЧИСЛО(20, 2))
		|	КОНЕЦ                                          КАК ПроцентОплаты,
		|	ВЫБОР КОГДА ЕСТЬNULL(СУММА(Расчеты.СуммаОтгрузок), 0) = 0
		|		ИЛИ ЕСТЬNULL(МАКСИМУМ(Расчеты.СуммаПервоначальныхВзаиморасчетов), 0) = 0
		|			ТОГДА 0
		|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(Расчеты.СуммаОтгрузок) * 100 / МАКСИМУМ(Расчеты.СуммаПервоначальныхВзаиморасчетов) КАК ЧИСЛО(20, 2))
		...
		|			ТОГДА ВЫРАЗИТЬ(СУММА(Расчеты.СуммаЗадолженности) * 100 / МАКСИМУМ(Расчеты.СуммаПервоначальныхВзаиморасчетов) КАК ЧИСЛО(20, 2))
		|		ИНАЧЕ - ВЫРАЗИТЬ(СУММА(Расчеты.СуммаЗадолженности) * 100 / МАКСИМУМ(Расчеты.СуммаПервоначальныхВзаиморасчетов) КАК ЧИСЛО(20, 2))
		...
		|		(&ШаблонПоляОплачено) * ВтСвязьЗаказовНакладных.ДоляЗаказа КАК СуммаОплатПоЗаказу,
		|		&ШаблонПоляСписание * ВтСвязьЗаказовНакладных.ДоляЗаказа   КАК СуммаСписаний,
		|		&ШаблонПоляОтгружено * ВтСвязьЗаказовНакладных.ДоляЗаказа  КАК СуммаОтгрузок,
		|		ВЫБОР КОГДА РасчетыПоСрокам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|			ТОГДА РасчетыПоСрокам.Долг - РасчетыПоСрокам.Предоплата
		|			ИНАЧЕ РасчетыПоСрокам.Предоплата - РасчетыПоСрокам.Долг
		|		КОНЕЦ * ВтСвязьЗаказовНакладных.ДоляЗаказа                 КАК СуммаЗадолженности,
