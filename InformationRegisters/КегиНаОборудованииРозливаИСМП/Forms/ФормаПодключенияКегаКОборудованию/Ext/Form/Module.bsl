...
Процедура ОбновитьПредставлениеСтатуса()
	...
	Если Не (Документы.ПодключениеКегаКОборудованиюРозливаИСМП.СтатусыОшибок().Найти(СтатусИСМП) = Неопределено
		Или Статус = Перечисления.СтатусыПодключенияКОборудованиюРозливаИСМП.Отключено) Тогда
		ДанныеСтатуса.Добавить(
...
Функция СоздатьПодключениеКегаКОборудованиюНаСервере()
	...
	Попытка
		...
		Если ОтключитьОтОборудования И ЗначениеЗаполнено(КодМаркировкиОтключить) Тогда
			...
			НоваяСтрока.КодМаркировки = КодМаркировкиОтключить;
...
Процедура КодМаркировкиПриИзмененииНаСервере()
	...
	Если ЗначенияРеквизитов.ВидПродукции <> ВидПродукции Тогда
		...
		ВывестиСообщениеОбОшибке(ЭтотОбъект,
		    СтрШаблон(НСтр(
				"ru = 'Вид продукции ""%1"" выбранного кода маркировки не соответствует виду продукции подключения к оборудованию ""%2""'"),
