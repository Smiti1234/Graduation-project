...
Функция ТекстЗапроса()
	...
	|		0                                               КАК ПлановаяПоставка
	|	ИЗ
	|		РегистрНакопления.РасчетыСПоставщикамиПланОплат.ОстаткиИОбороты({&НачалоПериода},{&КонецПериода},,,Не НераспределенныйАванс) КАК РасчетыПланОплатОстатки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаУчета
	...
	|	ИЗ
	|		РегистрНакопления.РасчетыСПоставщикамиПланПоставок.ОстаткиИОбороты({&НачалоПериода},{&КонецПериода},,,НЕ НераспределенныйАванс) КАК РасчетыПланПоставокОстатки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаУчета
	...
	|	ГДЕ
	|		АналитикаУчета.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|		И НЕ РасчетыПланОплат.НераспределенныйАванс
	|	{ГДЕ
