...
Функция ТаблицаРасходаЗаПериодТоварыНаСкладах(Настройки, ТаблицаЦен, Склад, Номенклатура, Характеристика)
	...
	Если ЕстьИсключаемыеРасходыТоварыНаСкладах Тогда
		...
		Для Каждого ДокументИмя Из МассивИсключаемыхДокументов Цикл
			...
			Если ДокументИмя = "РасходныйОрдерНаТовары" Тогда
				ОтборПоРасходнымОрдерам = 
				"Выбор Когда ТоварыНаСкладахОбороты.Регистратор ССЫЛКА Документ.РасходныйОрдерНаТовары Тогда
				|ВЫРАЗИТЬ(ТоварыНаСкладахОбороты.Регистратор КАК Документ.РасходныйОрдерНаТовары).СкладскаяОперация = ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаПоПеремещению)
				|Иначе
				|НЕ ТоварыНаСкладахОбороты.Регистратор ССЫЛКА Документ." + ДокументИмя + " Конец";
			Иначе
				ОтборПоРасходнымОрдерам = "НЕ ТоварыНаСкладахОбороты.Регистратор ССЫЛКА Документ." + ДокументИмя;
			КонецЕсли;
			
			СтрокаЗаменыРегистратор = СтрокаЗаменыРегистратор + ?(СтрокаЗаменыРегистратор = "", "", " И ") + ОтборПоРасходнымОрдерам;
