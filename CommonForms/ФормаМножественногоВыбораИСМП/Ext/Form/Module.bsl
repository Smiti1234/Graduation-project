...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КоличествоВключенныхТоварныхГрупп > 0 Тогда
		...
		Для Каждого ОбязательнаяГруппа Из СписокУчитываемойПродукцииТребующейОбязательнойПроверкиПриПродажеВРозницу Цикл
			...
			ТекстКомментария = ТекстКомментария + РазделительТекстов + ОбязательнаяГруппа.Представление;
			
		КонецЦикла;
		
		ТекстКомментария = ТекстКомментария + Символы.ПС + НСтр("ru = 'Настройка учитываемых видов продукции находится в разделе ""Настройка интеграции с ИС МП"".'");
