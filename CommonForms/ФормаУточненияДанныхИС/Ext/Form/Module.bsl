...
Процедура ОбновитьПараметрыЧастичногоВыбытия(Инициализация = Ложь)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыСканирования, "ВидОперацииИСМП") Тогда
		Возврат;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыЧастичногоВыбытия(Форма)
	...
	Элементы.РежимВыбытия.ТолькоПросмотр            = ЭтоЧастичноеВыбытиеПоДаннымГИСМТ Или Форма.КоличествоКодовМаркировкиНаОборудованииРозлива > 0 Или ЭтоВыборКодаМаркировкиКегаНаОборудованииРозливаИСМП(Форма);
