...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Удаляет движения документов ""Пересчетов товаров"" с незаполненными ресурсами.
		...
		|Исправляет движения документов закупки при неотфактурованных поставках работ.
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|		И ИСТИНА В(
		|			ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА КАК ЕстьДвиженияСПутымПодразделением
		|			ИЗ
		|				РегистрНакопления.ЗапасыИПотребности КАК Движения
		|			ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого ТекущиеДанные Из ОбновляемыеДанные Цикл
		...
		ИначеЕсли ТипЗнч(ТекущиеДанные.Регистратор) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
			...
			Попытка
				...
				Операция = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Регистратор, "ХозяйственнаяОперация");
				ПустоеПодразделение = Справочники.СтруктураПредприятия.ПустаяСсылка();
				Если Операция = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки
						Или Операция = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки Тогда
						
					Для Счетчик = 1 По ВсегоЗаписей Цикл
						ТекущаяЗапись = НаборЗаписей[ВсегоЗаписей - Счетчик];
						Если ТекущаяЗапись.Склад = ПустоеПодразделение Тогда
							НаборЗаписей.Удалить(ТекущаяЗапись);
						КонецЕсли;
					КонецЦикла;
		...
		ИначеЕсли ТипЗнч(ТекущиеДанные.Регистратор) = Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения") Тогда
			
			Продолжить;
			
		Иначе
			
			Если ТипЗнч(ТекущиеДанные.Регистратор) = Тип("ДокументСсылка.Сторно")
				Или ТипЗнч(ТекущиеДанные.Регистратор) = Тип("ДокументСсылка.СборкаТоваров")
				...
				Попытка
					...
					Если ТипЗнч(ТекущиеДанные.Регистратор) = Тип("ДокументСсылка.Сторно") Тогда
						СторнируемыйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Регистратор,
							"СторнируемыйДокумент");
						ИсправляемыйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СторнируемыйДокумент,
