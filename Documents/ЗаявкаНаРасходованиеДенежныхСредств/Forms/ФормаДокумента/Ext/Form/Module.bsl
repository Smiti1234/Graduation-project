	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Если ТипДокумента = Тип("ДокументСсылка.ЗаказПоставщику")
			...
			Если ИспользоватьВыплатыСамозанятым
				И (ТипДокумента = Тип("ДокументСсылка.ЗаказПоставщику")
				ИЛИ ТипДокумента = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")) Тогда
				СписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
			КонецЕсли;
			//-- Локализация
...
Процедура ПроверитьСоответствиеВалют()
	...
	ИначеЕсли Объект.ФормаОплатыЗаявки = Перечисления.ФормыОплаты.Безналичная Тогда
		...
		ВалютаСравнения = ВалютаПланированияДенежныхСредств(ВалютаСчетаКонтрагента);
		
		Если ВалютаСчета <> ВалютаСравнения Тогда
			Объект.БанковскийСчет = Неопределено;
		КонецЕсли;
