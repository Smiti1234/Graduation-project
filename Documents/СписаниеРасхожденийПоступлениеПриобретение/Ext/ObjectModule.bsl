...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Склад) Тогда
		Для ТекИндекс = 0 По Расхождения.Количество()-1 Цикл
			...
			Если РаботыУслуги.Найти(СтрокаТовары.Номенклатура.ТипНоменклатуры) = Неопределено Тогда
...
Функция ПараметрыЗаполненияВидовЗапасов(ПоПоложительномуОстатку, ПоСобственномуТипуЗапасов)
	...
	ПараметрыЗаполнения.ИмяТаблицыОстатков = ?(ПоПоложительномуОстатку, "ТоварыУПартнеровПоложительныйОстаток", "ТоварыУПартнеровОтрицательныйОстаток");
