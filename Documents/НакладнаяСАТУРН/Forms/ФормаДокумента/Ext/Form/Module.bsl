...
Процедура ПриСозданииЧтенииНаСервере(ОбновитьКомандыОснования = Истина)
	...
	ГрузополучательСопоставленСОрганизацией = ИнтеграцияСАТУРНВызовСервера.ОрганизацияСАТУРНСопоставленаСОрганизациейИБ(
		Объект.ГрузополучательОрганизацияСАТУРН).Сопоставлено;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация Или СтруктураРеквизитов.Свойство("Операция") Тогда
		...
		Элементы.ГрузополучательОрганизацияСАТУРН.Видимость        = ЭтоПеревозка Или ЭтоЭкспорт;
		Элементы.НадписьГрузополучательОрганизацияСАТУРН.Видимость = ЭтоПеревозка Или ЭтоЭкспорт;
...
Процедура ЗаполнитьГиперссылкиРеквизитов(ИмяХозяйствующегоСубъекта = Неопределено)
	...
	Если ИмяХозяйствующегоСубъекта = Неопределено Тогда
		...
		ПараметрПредставления.ОрганизацияСАТУРН = Объект.ГрузополучательОрганизацияСАТУРН;
	...
	Иначе
		...
		ПараметрПредставления.ОрганизацияСАТУРН = Объект[ИмяХозяйствующегоСубъекта + "ОрганизацияСАТУРН"];
