...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	Если ИмяРегистра = "РеестрДокументов" Тогда
		...
		ТекстыЗапросаВременныхТаблиц.Вставить(
			"ПриобретениеТоваровУслугРасходовано",
			ТекстЗапросаВременнаяТаблицаИзрасходованоПриобретениеТоваровУслуг(Запрос, ТекстыЗапроса));
	...
	Если ИмяРегистра = "РеестрДокументов" Тогда
		ТекстЗапроса = ОбновлениеИнформационнойБазыУТ.АдаптироватьЗапросПроведенияПоНезависимомуРегистру(
			ТекстЗапроса, ПолноеИмяДокумента, СинонимТаблицыДокумента, Истина, , ТекстыЗапросаВременныхТаблиц);
	Иначе
...
Функция ТекстЗапросаТаблицаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТекстыЗапроса, Регистры)
	...
	|	
	|	ПрочиеРасходы.Сумма КАК Сумма,
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаУпр,
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаРегл,
	|	
...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	КОНЕЦ КАК Аналитика,
	|
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК Сумма
	|ИЗ
	|	Документ.АвансовыйОтчет.ПрочиеРасходы КАК ПрочиеРасходы
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКурсыВалютСтрокОтчета КАК ТаблицаКурсыВалютСтрокОтчета
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваДоходыРасходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|	ТаблицаПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК Сумма,
	|	ТаблицаПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаРегл,
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ПриобретениеТоваровУслугРасходовано", ТекстыЗапроса) Тогда
		ТекстЗапросаВременнаяТаблицаИзрасходованоПриобретениеТоваровУслуг(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	//-- Локализация
	|	+ ЕСТЬNULL(ПриобретениеТоваровУслугИзрасходовано.Сумма, 0) КАК Сумма,
	|	&Валюта                                 КАК Валюта,
	...
	|ИЗ
	|	Документ.АвансовыйОтчет КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПриобретениеТоваровУслугРасходовано КАК ПриобретениеТоваровУслугИзрасходовано
	|		ПО ДанныеДокумента.Ссылка = ПриобретениеТоваровУслугИзрасходовано.Ссылка
	//++ Локализация
...
Функция ТекстЗапросаВременнаяТаблицаИзрасходованоПриобретениеТоваровУслуг(Запрос, ТекстыЗапроса)

	ИмяРегистра = "ПриобретениеТоваровУслугРасходовано";
