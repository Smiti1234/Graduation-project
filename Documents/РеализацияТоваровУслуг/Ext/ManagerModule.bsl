...
Функция ТекстЗапросаВтАгентскиеУслуги(Запрос, ТекстыЗапроса)
	...
	|			ТОГДА Услуги.Ссылка
	|		КОГДА НЕ СоглашениеСПоставщиком.Ссылка ЕСТЬ NULL
	...
	|			ПО КОформлению.Ссылка = ДанныеДокументаШапка.Ссылка
	|
	...
	|		И СоглашениеСПоставщиком.ХозяйственнаяОперация
	|				= ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОказаниеАгентскихУслуг)
	|		И СоглашениеСПоставщиком.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСПоставщиками.Действует)
	...
	|	ПО СоглашениеЗакрытое.Партнер = ЕСТЬNULL(КОформлению.ХарактеристикаПринципал, КОформлению.НоменклатураПринципал)
