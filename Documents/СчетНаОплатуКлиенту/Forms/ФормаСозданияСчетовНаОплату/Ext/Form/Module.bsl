...
Процедура ОбновитьСервер()
	...
	Иначе
		...
		Запрос.Текст = Запрос.Текст + ТекстЗапросаУменьшенияПланаОплаты();
		...
		ВыборкаУменьшениеПланаОплаты = МассивРезультатов[2].Выбрать();
		ВыборкаШапка                 = МассивРезультатов[3].Выбрать();
		...
		// Вычтем из строк сверх заказа уменьшение К оплате, которое делает накладная
		ТаблицаДокументовЭтапы.Индексы.Добавить("Документ");
		Пока ВыборкаУменьшениеПланаОплаты.Следующий() Цикл
			СтрокиЭтаповПоДокументу = ТаблицаДокументовЭтапы.НайтиСтроки(Новый Структура("Документ", ВыборкаУменьшениеПланаОплаты.Документ));
			Если СтрокиЭтаповПоДокументу.Количество() > 0 Тогда
				СуммыКОплате = ТаблицаДокументовЭтапы.Скопировать(СтрокиЭтаповПоДокументу, "СуммаКОплате").ВыгрузитьКолонку("СуммаКОплате");
				СуммыУменьшения = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(ВыборкаУменьшениеПланаОплаты.СуммаУменьшения, СуммыКОплате);
				Если СуммыУменьшения <> Неопределено Тогда
					Для ИндексСтроки = 0 По СуммыУменьшения.Количество() - 1 Цикл
						СтрокиЭтаповПоДокументу[ИндексСтроки].СуммаКОплате = СтрокиЭтаповПоДокументу[ИндексСтроки].СуммаКОплате - СуммыУменьшения[ИндексСтроки];
						СтрокиЭтаповПоДокументу[ИндексСтроки].СуммаПлатежа = СтрокиЭтаповПоДокументу[ИндексСтроки].СуммаПлатежа - СуммыУменьшения[ИндексСтроки];
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		ТаблицаДокументовЭтапы.Свернуть("Порядок,Выбран,Оплачена,ИндексКартинки,ДатаПлатежа,ПроцентПлатежа,ЭтоЗалогЗаТару,ЭтапСверхЗаказа", "СуммаПлатежа,СуммаКОплате");
