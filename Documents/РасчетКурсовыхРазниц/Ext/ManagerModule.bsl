		...
		Попытка
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов") Тогда
				Для Каждого Организация Из ДанныеКПереоценкеЗаМесяц.Организация Цикл
					ПереоценитьКредитыДепозитыЗаймы(Организация, НачалоРасчета);
				КонецЦикла;
			КонецЕсли;
		Исключение
...
Функция ТекстЗапросаКурсовыеРазницыПроцентныеРасходыДисконтирования()
	...
	|		ПО (ИСТИНА)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&КонецПериода) КАК КурсДоговора
...
Функция ТекстЗапросаКурсовыеРазницыТоварыКОформлениюОтчетовКомитенту()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&КонецПериода, Валюта = &ВалютаУправленческогоУчета И БазоваяВалюта = &ВалютаРегламентированногоУчета) КАК КурсВалютыУпр
	|			ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&КонецПериода, ) КАК КурсыВалютПоДоговорам
...
Функция ЗаписатьДвижения(ДокументСсылка, МенеджерТаблиц, ИмяРегистра, День, ДополнительныеПараметры)
	...
	ВсегоЗаписано = 0;
	РазрешенаЗапись = НЕ ДополнительныеПараметры.ПерезаполнениеРегистровНУ;
