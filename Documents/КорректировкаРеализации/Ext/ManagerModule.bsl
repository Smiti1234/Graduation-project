...
Процедура СформироватьВременнуюТаблицуИсходныхДанных(МенеджерВременныхТаблиц, ДокументОснование, ДокументСсылка = Неопределено) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПервичныйДокумент") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КоличествоПоРНПТ,", "ТаблицаТовары.КоличествоПоРНПТ КАК КоличествоПоРНПТ,");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НомерГТД,", "ТаблицаТовары.НомерГТД КАК НомерГТД,");
		ТекстЗапросаПоОснованию = "";
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КоличествоПоРНПТ,", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НомерГТД,", "");
...
Функция ТекстЗапросаВТПроверкиДанныхПоПоследнейКорректировке()
	...
	|	МАКСИМУМ(КорректировкаРеализацииТовары.Ссылка.Дата) КАК Дата,
	...
	|	КорректировкаРеализацииТовары.СтатьяДоходов КАК СтатьяДоходов,
	|	КорректировкаРеализацииТовары.АналитикаДоходов КАК АналитикаДоходов,
	...
	|	КорректировкаРеализацииТовары.Подразделение КАК Подразделение
	...
	|					ТОГДА ВТРасхождения.Номенклатура = КорректировкаРеализацииТовары.Номенклатура
	|							И ВТРасхождения.Характеристика = КорректировкаРеализацииТовары.Характеристика
	|							И ВТРасхождения.Упаковка = КорректировкаРеализацииТовары.Упаковка
	|							И ВТРасхождения.НоменклатураНабора = КорректировкаРеализацииТовары.НоменклатураНабора
	|							И ВТРасхождения.ХарактеристикаНабора = КорректировкаРеализацииТовары.ХарактеристикаНабора
	|							И ВТРасхождения.СтатьяДоходов = КорректировкаРеализацииТовары.СтатьяДоходов
	|							И ВТРасхождения.АналитикаДоходов = КорректировкаРеализацииТовары.АналитикаДоходов
	|							И ВТРасхождения.Серия = КорректировкаРеализацииТовары.Серия
	|							И ВТРасхождения.Склад = КорректировкаРеализацииТовары.Склад
	|							И ВТРасхождения.НомерГТД = КорректировкаРеализацииТовары.НомерГТД
	|							И ВТРасхождения.Подразделение = КорректировкаРеализацииТовары.Подразделение
	|				ИНАЧЕ ВТРасхождения.Содержание = КорректировкаРеализацииТовары.Содержание
	|						И ВТРасхождения.Подразделение = КорректировкаРеализацииТовары.Подразделение
	|						И ВТРасхождения.СтатьяДоходов = КорректировкаРеализацииТовары.СтатьяДоходов
	|						И ВТРасхождения.АналитикаДоходов = КорректировкаРеализацииТовары.АналитикаДоходов
	|			КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовСторноИсправление
	|		ПО КорректировкаРеализацииТовары.Ссылка = РеестрДокументовСторноИсправление.СторнируемыйДокумент
	|			И (РеестрДокументовСторноИсправление.Проведен)
	|			И (НЕ РеестрДокументовСторноИсправление.ДополнительнаяЗапись)
	|ГДЕ
	|	КорректировкаРеализацииТовары.Ссылка.ДокументОснование = &ДокументОснование
	|	И КорректировкаРеализацииТовары.Ссылка <> &Ссылка
	|	И КорректировкаРеализацииТовары.Ссылка.Проведен
	|	И КорректировкаРеализацииТовары.Ссылка.ТипКорректировки = &ТипКорректировки
	|	И КорректировкаРеализацииТовары.Ссылка.Дата < &Дата
	|	И РеестрДокументовСторноИсправление.Ссылка ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаРеализацииТовары.НоменклатураНабора,
	|	КорректировкаРеализацииТовары.ХарактеристикаНабора,
	|	КорректировкаРеализацииТовары.СтатьяДоходов,
	|	КорректировкаРеализацииТовары.АналитикаДоходов,
	|	КорректировкаРеализацииТовары.Подразделение,
	...
	|	КорректировкаРеализацииТовары.Номенклатура КАК Номенклатура,
	|	КорректировкаРеализацииТовары.НоменклатураПартнера КАК НоменклатураПартнера,
	|	КорректировкаРеализацииТовары.Характеристика КАК Характеристика,
	|	КорректировкаРеализацииТовары.НоменклатураНабора КАК НоменклатураНабора,
	|	КорректировкаРеализацииТовары.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	КорректировкаРеализацииТовары.Содержание КАК Содержание,
	...
	|		ПО (ВЫБОР
	|				КОГДА &ЭтоКорректировкаДоВводаОстатковТоваровИУслуг
	|					ТОГДА ПоследняяКорректировка.Номенклатура = КорректировкаРеализацииТовары.Номенклатура
	|							И ПоследняяКорректировка.Характеристика = КорректировкаРеализацииТовары.Характеристика
	|							И ПоследняяКорректировка.Содержание = КорректировкаРеализацииТовары.Содержание
	|							И ПоследняяКорректировка.Упаковка = КорректировкаРеализацииТовары.Упаковка
	|							И ПоследняяКорректировка.НоменклатураНабора = КорректировкаРеализацииТовары.НоменклатураНабора
	|							И ПоследняяКорректировка.ХарактеристикаНабора = КорректировкаРеализацииТовары.ХарактеристикаНабора
	|							И ПоследняяКорректировка.СтатьяДоходов = КорректировкаРеализацииТовары.СтатьяДоходов
	|							И ПоследняяКорректировка.АналитикаДоходов = КорректировкаРеализацииТовары.АналитикаДоходов
	|							И ПоследняяКорректировка.Серия = КорректировкаРеализацииТовары.Серия
	|							И ПоследняяКорректировка.Склад = КорректировкаРеализацииТовары.Склад
	|							И ПоследняяКорректировка.НомерГТД = КорректировкаРеализацииТовары.НомерГТД
	|							И ПоследняяКорректировка.Подразделение = КорректировкаРеализацииТовары.Подразделение
	|							И ПоследняяКорректировка.Дата = КорректировкаРеализацииТовары.Ссылка.Дата
	|				ИНАЧЕ ПоследняяКорректировка.Содержание = КорректировкаРеализацииТовары.Содержание
	|						И ПоследняяКорректировка.СтатьяДоходов = КорректировкаРеализацииТовары.СтатьяДоходов
	|						И ПоследняяКорректировка.АналитикаДоходов = КорректировкаРеализацииТовары.АналитикаДоходов
	|						И ПоследняяКорректировка.Подразделение = КорректировкаРеализацииТовары.Подразделение
	|						И ПоследняяКорректировка.Дата = КорректировкаРеализацииТовары.Ссылка.Дата
