...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС") Тогда
		...
		Если КомандаСозданияСчетФактуры <> Неопределено Тогда
			ОплатаСамозанятому = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому;
...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	Финансы.НовыйОтбор(ГруппНе, "ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.КонвертацияВалюты);
	Элемент.Свойства.Вставить("Видимость");
	#КонецОбласти
	...
	Финансы.НовыйОтбор(Элемент.Условие, "ПлатежСКонвертацией", Истина);
	Элемент.Свойства.Вставить("Заголовок", НСтр("ru = 'Сумма в валюте получателя'"));
...
Функция ТекстЗапросаВременнаяТаблицаРасшифровкаПлатежа(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено) Экспорт
	...
	|		ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.Сумма * &КоэффициентПересчетаВВалютуДенежныхСредств КАК ЧИСЛО(31,2))
	|	ИНАЧЕ
	...
	|
	|	ВЫБОР КОГДА ТаблицаРасшифровкаПлатежа.Ссылка.ПлатежСКонвертацией ТОГДА
	|		ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.Сумма * &КоэффициентПересчетаВВалютуДенежныхСредств КАК ЧИСЛО(31,2))
	|	ИНАЧЕ
	|		ТаблицаРасшифровкаПлатежа.Сумма
	|	КОНЕЦ КАК Сумма,
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено, ДокументОбъект = Неопределено) Экспорт
	...
	|	ВЫБОР КОГДА ТаблицаДокумента.ПлатежСКонвертацией ТОГДА
	|		ЕСТЬNULL(ВЫРАЗИТЬ(Строки.Сумма * &КоэффициентПересчетаВВалютуДенежныхСредств КАК ЧИСЛО(31,2)), ТаблицаДокумента.СуммаВВалютеОтправителя)
	|	ИНАЧЕ
	|		ЕСТЬNULL(Строки.Сумма, ТаблицаДокумента.СуммаДокумента)
	...
	|	ТаблицаДокумента.СуммаВВалютеОтправителя КАК СуммаБезНДС,
	|	НЕОПРЕДЕЛЕНО КАК СтавкаНДС,
	|	0 КАК СуммаНДС,
	|	0 КАК СуммаВзаиморасчетов,
	|	0 КАК СуммаНДСВзаиморасчетов,
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если (ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеТаможне
		ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому) Тогда
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	ТекстВозврат = "
		...
		|				И Таблица.ОбъектРасчетовОбъект <> Таблица.Ссылка
	...
	ТекстУменьшитьОплачивается = "
		...
		|	И Таблица.ОбъектРасчетов <> Документ.ОбъектРасчетов
		|";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение статей ДДС в документе ""Списание безналичных денежных средств"" в табличной части ""Лицевые счета сотрудников"".'");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.СписаниеБезналичныхДенежныхСредств.ОбработатьКолонкуКонтрагентВРасшифровкеПлатежа";
	Обработчик.Версия = "11.5.15.22";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ce00f8dd-e5e4-4ba2-9d6d-0c1f952dfdbd");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления =
		"Документы.СписаниеБезналичныхДенежныхСредств.ЗарегистрироватьДанныеДляЗаполненияКолонкиКонтрагент";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий =
		НСтр("ru = 'Заполнение контрагента в табличной части ""Расшифровка платежа"" документа ""Списание безналичных денежных средств"".'");
