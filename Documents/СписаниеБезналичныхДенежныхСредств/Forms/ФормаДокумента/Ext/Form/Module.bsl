...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если Объект.ПлатежСКонвертацией И МассивРеквизитовОперации.Найти("ПлатежСКонвертацией") = Неопределено Тогда
...
Функция БанковскийСчетПриИзмененииКлиент(ИмяОбработчика)
	...
	Если Не ФинансыКлиент.НеобходимПересчетВВалюту(Объект, Объект.Валюта, РеквизитыБанковскогоСчета.Валюта)
		Или Объект.ПлатежСКонвертацией 
			И (ЗначениеЗаполнено(Объект.БанковскийСчетКонтрагента) 
				Или ЗначениеЗаполнено(Объект.БанковскийСчетПолучатель)) Тогда
...
Процедура БанковскийСчетПриИзмененииСервер(СтруктураРеквизитов, ПересчитыватьСуммы)
	...
	Если Объект.ПлатежСКонвертацией Тогда
		Если ВалютаБанковскогоСчета <> СтруктураРеквизитов.Валюта Тогда
			Объект.СуммаВВалютеОтправителя = 0;
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ПлатежСКонвертацией Тогда
		Если ЗначениеЗаполнено(Объект.БанковскийСчетКонтрагента) Тогда
			СтруктураРеквизитов.Вставить("Валюта",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчетКонтрагента, "ВалютаДенежныхСредств"));
		ИначеЕсли ЗначениеЗаполнено(Объект.БанковскийСчетПолучатель) Тогда
			СтруктураРеквизитов.Вставить("Валюта",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчетПолучатель, "ВалютаДенежныхСредств"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущийБанковскийСчет) Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяВалюта) И ИзмениласьВалюта Тогда
			...
			Если Не Объект.ПлатежСКонвертацией Тогда
				Объект.БанковскийСчетКонтрагента = Справочники.БанковскиеСчетаКонтрагентов.ПустаяСсылка();
				Объект.БанковскийСчетПолучатель  = Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
			КонецЕсли;
	...
	ДенежныеСредстваСервер.ОбновитьВалютуВзаиморасчетовРасшифровкиПлатежа(
		"Документ.СписаниеБезналичныхДенежныхСредств", Объект, ВалютаРеглУчета, ИзмениласьВалюта);
	
	
	Если МассивРеквизитовОперации.Найти("ПлатежСКонвертацией") = Неопределено Тогда
		Объект.ПлатежСКонвертацией = Ложь;
	Иначе
...
Процедура БанковскийСчетКонтрагентаПриИзмененииСервер()
	...
	// Нужно пересчитать суммы
	Если ТекущаяВалюта <> Объект.Валюта 
		И (Объект.ПлатежСКонвертацией Или ТекущийПлатежСКонвертацией)Тогда
...
Процедура ПодобратьПодарочныйСертификат(Команда)
	...
	Если ПодарочныеСертификатыКлиент.ПроверитьВозможностьДобавленияПодарочногоСертификата(ЭтотОбъект, Ложь) Тогда
		...
		Если Объект.Проведен Тогда
			ПодарочныеСертификатыКлиент.ОтобразитьВопросОНеобходимостиНепроведенногоДокумента(ЭтотОБъект, ОписаниеОповещенияЗаверщения);
		Иначе
...
Процедура ПодборПоОстаткамПослеПроверки(Результат, ДополнительныеПараметры) Экспорт 
	...
	//++ Локализация
	ДенежныеСредстваКлиентЛокализация.ДополнитьОтборОбъектовРасчета(ЭтотОбъект, ДополнительныеОтборы);
	//-- Локализация
...
Процедура ПодобратьИзЗаявокЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику")
		...
		ПараметрыПодбора.Вставить("ПодотчетноеЛицо", СписокПодотчетныхЛиц);
...
Процедура ПолучитьДоговорПоУмолчаниюСервер(Контрагент, БанковскийСчетКонтрагента, ЗаполнятьСчетПоДоговору = Истина)
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов) Тогда
		...
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому Тогда
			ХозяйственнаяОперацияДоговора.Добавить(Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика);
		Иначе
			Возврат;
		КонецЕсли;
		...
		ДоговорКонтрагента = ДенежныеСредстваСервер.ДоговорПоУмолчанию(
			ДоговорКонтрагента, Партнер, Контрагент, Объект.Организация, ХозяйственнаяОперацияДоговора);
		...
		Если ЗаполнятьСчетПоДоговору Тогда
			...
			Если Объект.Валюта <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчетКонтрагента, "ВалютаДенежныхСредств") Тогда
...
Процедура УстановитьВидимостьКнопкиПодбораПодарочногоСертификата()
	...
	РазрешенаПродажаПодарочногоСертификата =
		РозничныеПродажиЛокализация.РазрешенаПродажаПодарочногоСертификатаВДокументе(Объект.ХозяйственнаяОперация, Объект.Контрагент);
...
Процедура ВыгрузитьФайлДляОбменаСБанкомЛокализация(Команда)
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = 
		ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОплатаСамозанятому") Тогда
...
Процедура ЗаполнитьСписокХозяйственныхОперацийЛокализация()
	...
	ДоступныеОперации.Добавить(Операции.ВыплатаЗарплатыНаЛицевыеСчета);
	ДоступныеОперации.Добавить(Операции.ОплатаАрендодателю);
	ДоступныеОперации.Добавить(Операции.ОплатаСамозанятому);
...
Процедура УточнитьСписокХозяйственныхОперацийЛокализация()
	...
	ДенежныеСредстваСерверЛокализация.УстановитьВидимостьОперацииОплатаСамозанятому(ЭлементХозяйственнаяОперация);
...
Процедура ИнициализироватьБанковскиеСчетаСпискаКонтрагентов()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому Тогда
