...
Процедура НастроитьВидимостьКомандыЗаполнитьПодразделение();
	ВидимостьКоманды = НЕ (ХозяйственныеОперацииРаздельнойЗакупки(Ложь, Истина).Найти(Объект.ХозяйственнаяОперация) <> Неопределено
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ВариантОформленияЗакупок") = Перечисления.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваровИУслуг);
