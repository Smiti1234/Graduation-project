...
Функция ТекстЗапросаВТКурсыВалютСтрокОтчета(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено)
	...
	|	Товары.СуммаИтог КАК СуммаИтог
	|ПОМЕСТИТЬ СтрокиОтчета
	|ИЗ
	|	ДанныеДокументаТаблицаТовары КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокумента
	|			ПО Товары.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	...
	|
	|ГДЕ
	|	СтрокиОтчета.СуммаИтог <= КурсыВалютАвансов.Получено
	...
	|ВЫБРАТЬ
	|	СтрокиОтчета.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	&КоэффициентПересчетаВВалютуУпр КАК КоэффициентПересчетаУпр,
	|	&КоэффициентПересчетаВВалютуРегл КАК КоэффициентПересчетаРегл
	|ИЗ
	|	СтрокиОтчета КАК СтрокиОтчета
	|		
	...
	|		(СтрокиОтчета.СуммаИтог - КурсыВалютАвансов.Получено) * &КоэффициентПересчетаВВалютуУпр КАК СуммаУпр,
	|		(СтрокиОтчета.СуммаИтог - КурсыВалютАвансов.Получено) * &КоэффициентПересчетаВВалютуРегл КАК СуммаРегл
	|	ИЗ
	|		СтрокиОтчета КАК СтрокиОтчета
	|		
	|			ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютАвансов КАК КурсыВалютАвансов
	...
	|		СтрокиОтчета.СуммаИтог - СтрокиОтчета.Сумма < ЕСТЬNULL(КурсыВалютАвансов.Получено, 0)
	|		И СтрокиОтчета.СуммаИтог > ЕСТЬNULL(КурсыВалютАвансов.Получено, 0)
	|		И КурсыВалютАвансов.КоэффициентПересчетаУпр ЕСТЬ NULL
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса, ДокументОбъект)
