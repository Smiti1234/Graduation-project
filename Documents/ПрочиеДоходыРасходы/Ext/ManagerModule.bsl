...
Функция ТекстЗапросаВтДанныеРасходы(Запрос, ТекстыЗапроса) Экспорт
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(
	|				ДанныеДокументаРасходы.Сумма 
	|				* ВЫРАЗИТЬ(ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(10,4)
	|			) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ                                      КАК Сумма,
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация В (
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(
	|				ДанныеДокументаРасходы.СуммаБезНДС 
	|				* ВЫРАЗИТЬ(ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(10,4)
	|			) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК СуммаБезНДС,
...
Функция ТекстЗапросаВтДанныеДоходы(Запрос, ТекстыЗапроса) Экспорт
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеДокументаДоходы.Сумма 
	|				* ВЫРАЗИТЬ(ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(10,4)
	|			) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	...
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ДанныеДокументаДоходы.СуммаУпр 
	|				* ВЫРАЗИТЬ(ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуУпр, 0) КАК ЧИСЛО(10,4)
	|			) КАК ЧИСЛО(31, 2))
	|	КОНЕЦ КАК СуммаУпр,
	|	ВЫБОР
