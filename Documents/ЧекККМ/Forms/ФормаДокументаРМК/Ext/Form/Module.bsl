...
Процедура ПриИзмененииНоменклатурыХарактеристикиКоличестваЛокализация()
	...
	СтруктураПоиска = Новый Структура("НоменклатураПодключенаКОборудованиюРозливаИСМП", Истина);
	СтрокиТоваров = Объект.Товары.НайтиСтроки(СтруктураПоиска);
	
	Если СтрокиТоваров.Количество() Тогда
		
		ИнтеграцияИСУТ.ЗаполнитьКодыМаркировкиНаОборудованииРозливаИСМП(ЭтотОбъект, СтрокиТоваров);
...
&НаКлиенте
Процедура ТоварыПриОкончанииРедактированияЛокализация(Элемент, НоваяСтрока, ОтменаРедактирования, КэшированныеЗначения, ДополнительныеПараметры)
	
// ++ Локализация
	
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво"))
		И ТекущиеДанные.НоменклатураПодключенаКОборудованиюРозливаИСМП Тогда
		КэшированныеЗначения.Вставить("ЗаполнитьКодыМаркировкиКегНаОборудованииРозливаИСМП");
		ДополнительныеПараметры.Вставить("ТребуетсяСерверныйВызов");
	КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактированияНаСервереЛокализация(ИдентификаторСтроки, КэшированныеЗначения)
	...
	Если КэшированныеЗначения.Свойство("ЗаполнитьКодыМаркировкиКегНаОборудованииРозливаИСМП") Тогда
		
		ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
		Если ЗначениеЗаполнено(ТекущиеДанные.Количество) Тогда
			СтрокиТоваров = Новый Массив();
			СтрокиТоваров.Добавить(ТекущиеДанные);
			ИнтеграцияИСУТ.ЗаполнитьКодыМаркировкиНаОборудованииРозливаИСМП(ЭтотОбъект, СтрокиТоваров, Истина);
		КонецЕсли;
		КэшированныеЗначения.Удалить("ЗаполнитьКодыМаркировкиКегНаОборудованииРозливаИСМП");
...
Функция ИдентификаторыОперацииОплаты(ТорговаяТочка, ДокументОплаты)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДокументОплаты) Тогда
			ИдентификаторОплаты = ИнтеграцияСПлатежнымиСистемами.ИдентификаторыОперацииОплаты(ТорговаяТочка, ДокументОплаты);
		КонецЕсли;
