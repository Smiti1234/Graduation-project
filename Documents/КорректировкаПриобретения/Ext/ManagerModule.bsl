...
Функция МакетФормыВыбораНазначенийПоПараметру(ПараметрФормированияШаблоновНазначения = "") Экспорт
	...
	Если ПараметрФормированияШаблоновНазначения = "КорректировкаДоВоодаОстатков" Тогда
		...
		ПутьКРеквизитуНазначения = "ТаблицаКорректировки.Назначение";
	Иначе
		ПутьКРеквизитуНазначения = "Объект.Товары.Назначение";
	КонецЕсли;

	// Потребности в товарах на складе.
	ОписаниеКолонок = Справочники.Назначения.ДобавитьОписаниеКолонок(МакетФормы, "ОбеспечениеЗаказов", Истина, ПутьКРеквизитуНазначения);
	ОписаниеКолонок.Колонки.НайтиПоЗначению("Потребность").Пометка = Истина;
	ОписаниеКолонок.УсловиеИспользования = "Объект.Товары.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)";
	...
	// Потребности в работах в подразделении-получателе.
	ОписаниеКолонок = Справочники.Назначения.ДобавитьОписаниеКолонок(МакетФормы, "ОбеспечениеЗаказовРаботами", Истина, ПутьКРеквизитуНазначения);
	ОписаниеКолонок.Колонки.НайтиПоЗначению("Потребность").Пометка = Истина;
	ОписаниеКолонок.УсловиеИспользования = "Объект.Товары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)";
...
Функция КлючевыеРеквизитыСвязиСтрокТаблиц(КорректировкаПриобретенияУслугПрочихАктивов) Экспорт
	...
	Иначе
		КлючевыеРеквизиты = "Номенклатура,Характеристика,Серия,Упаковка,Склад,НомерГТД,СтатьяРасходов,АналитикаРасходов,Подразделение";
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			И ТаблицаДокумента.Ссылка.Договор.ВариантОформленияЗакупок = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеотфактурованныеПоставкиТоваров)
