...
Процедура ПодобратьПодарочныйСертификат(Команда)
	...
	Если ПодарочныеСертификатыКлиент.ПроверитьВозможностьДобавленияПодарочногоСертификата(ЭтотОбъект, Ложь) Тогда
		Если Объект.Проведен Тогда
			...
			ПодарочныеСертификатыКлиент.ОтобразитьВопросОНеобходимостиНепроведенногоДокумента(ЭтотОбъект, ОписаниеОповещенияЗавершения);
		Иначе
...
Процедура ОбновитьСлужебныеРеквизитыТабличнойЧасти(Вид)
	...
	Если Не ЭтоДебет Тогда
		...
		ОбъектыОбъектовРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ТаблицаОбъектовРасчетов.ВыгрузитьКолонку(0), "Объект");
		Для Каждого Строка Из Объект[ИмяТЧ] Цикл
			ОбъектОбъектаРасчетов = ОбъектыОбъектовРасчетов[Строка.ОбъектРасчетов];
			Строка.ОбъектРасчетовПодарочныйСертификат = ТипЗнч(ОбъектОбъектаРасчетов) = Тип("СправочникСсылка.ПодарочныеСертификаты")
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормыПоВидуОперации()
	...
	ПараметрыВыбораПолучательДтБезРазбиенияПартнер = Новый Массив;
	...
	Элементы.ПолучательДтБезРазбиенияПартнер.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораПолучательДтБезРазбиенияПартнер);
...
Процедура ОбъектРасчетовОбработкаВыбора(Вид, ВыбранноеЗначение)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачетаЗадолженности.ОтражениеОплатыЧерезКомиссионера")
			ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачетаЗадолженности.ОтражениеВозвратаОплатыЧерезКомиссионера") Тогда
			ЗаполнитьЗначенияСвойств(ТекущиеДанные, ВыбранноеЗначение, "ОбъектРасчетов, ВалютаВзаиморасчетов, СуммаВзаиморасчетов");
