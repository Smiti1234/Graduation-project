...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Бартер
		...
		Если Приемник.Количество() > 0 Тогда
			Приемник[0].СуммаРегл = СуммаРегл;
			Приемник[0].СуммаУпр = СуммаУпр;
			
			ВалютаРегл = ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Организация);
			ВалютаУпр = Константы.ВалютаУправленческогоУчета.Получить();
			Если Приемник[0].ВалютаВзаиморасчетов = ВалютаРегл И СуммаРегл <> 0 Тогда
				Приемник[0].СуммаВзаиморасчетов = СуммаРегл;
			ИначеЕсли Приемник[0].ВалютаВзаиморасчетов = ВалютаУпр И СуммаУпр <> 0 Тогда
				Приемник[0].СуммаВзаиморасчетов = СуммаУпр;
			ИначеЕсли Источник.Количество() > 0 И Приемник[0].ВалютаВзаиморасчетов = Источник[0].ВалютаВзаиморасчетов Тогда
				Приемник[0].СуммаВзаиморасчетов = Источник.Итог("СуммаВзаиморасчетов");
			КонецЕсли;
			Если Источник.Количество() > 0 И Источник[0].СуммаВзаиморасчетов = 0 И Приемник[0].СуммаВзаиморасчетов <> 0 Тогда
				Источник[0].СуммаВзаиморасчетов = Приемник[0].СуммаВзаиморасчетов;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
