...
Функция ТаблицаТоваров(ПараметрыЗаполнения) Экспорт
	...
	|ГДЕ
	|	НЕ &ОтборПоСчетуФактуре
	|		ИЛИ ВЫБОР
	|			КОГДА вт_ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.Исправление
	|				ТОГДА вт_ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.СчетФактураОснование
	|			ИНАЧЕ вт_ТаблицаТоваровПредварительная.ПредыдущийСчетФактура
	|		КОНЕЦ = &СчетФактураОтбора
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ОтборПоСчетуФактуре", ЗначениеЗаполнено(СчетФактураОтбора) И Не ЭтоПеревыставление);
	...
	ДополнительныеПараметры.Вставить("ОтборПоСчетуФактуре", ЗначениеЗаполнено(СчетФактураОтбора) И Не ЭтоПеревыставление);
	ДополнительныеПараметры.Вставить("СчетФактураОтбора", СчетФактураОтбора);
...
Функция ТаблицаТоваровИсправление(МенеджерВременныхТаблиц, ДополнительныеПараметры)
	...
	|ГДЕ
	|	НЕ &ОтборПоСчетуФактуре
	|	И (НЕ ТаблицаТоваровСФ.Количество + ТаблицаТоваровСФ.КоличествоУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.Количество, 0) = 0
	...
	|			И ТаблицаТоваровСФ.ЕдиницаИзмерения = ТаблицаТоваровДО.ЕдиницаИзмерения
	|			И ТаблицаТоваровСФ.СтавкаНДС = ТаблицаТоваровДО.СтавкаНДС
	|		ЛЕВОЕ СОЕДИНЕНИЕ вт_ТаблицаТоваровПредварительная КАК ТаблицаТоваровПредварительная
	|		ПО ТаблицаТоваровСФ.ИсходныйСчетФактура = ТаблицаТоваровПредварительная.ИсходныйСчетФактура
	|			И ТаблицаТоваровСФ.Номенклатура = ТаблицаТоваровПредварительная.Номенклатура
	|			И ТаблицаТоваровСФ.Характеристика = ТаблицаТоваровПредварительная.Характеристика
	|			И ТаблицаТоваровСФ.Серия = ТаблицаТоваровПредварительная.Серия
	|			И ТаблицаТоваровСФ.НоменклатураНабора = ТаблицаТоваровПредварительная.НоменклатураНабора
	|			И ТаблицаТоваровСФ.ХарактеристикаНабора = ТаблицаТоваровПредварительная.ХарактеристикаНабора
	|			И ТаблицаТоваровСФ.Содержание = ТаблицаТоваровПредварительная.Содержание
	|			И ТаблицаТоваровСФ.ТипЗапасов = ТаблицаТоваровПредварительная.ТипЗапасов
	|			И ТаблицаТоваровСФ.КодТНВЭД = ТаблицаТоваровПредварительная.КодТНВЭД
	|			И ТаблицаТоваровСФ.НомерГТД = ТаблицаТоваровПредварительная.НомерГТД
	|			И ТаблицаТоваровСФ.ЕдиницаИзмерения = ТаблицаТоваровПредварительная.ЕдиницаИзмерения
	|			И ТаблицаТоваровСФ.СтавкаНДС = ТаблицаТоваровПредварительная.СтавкаНДС
	|ГДЕ
	|	&ОтборПоСчетуФактуре
	|	И ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.Исправление, ЛОЖЬ)
	|			ТОГДА ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.СчетФактураОснование
	|		ИНАЧЕ
	|			ЕСТЬNULL(ТаблицаТоваровПредварительная.ПредыдущийСчетФактура, НЕОПРЕДЕЛЕНО)
	|	КОНЕЦ = &СчетФактураОтбора
	|	И (НЕ ТаблицаТоваровСФ.Количество + ТаблицаТоваровСФ.КоличествоУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.Количество, 0) = 0
	|	ИЛИ НЕ ТаблицаТоваровСФ.КоличествоПоРНПТ + ТаблицаТоваровСФ.КоличествоПоРНПТУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.КоличествоПоРНПТ, 0) = 0
	|	ИЛИ НЕ ТаблицаТоваровСФ.Сумма + ТаблицаТоваровСФ.СуммаУменьшение + ЕСТЬNULL(ТаблицаТоваровДО.Сумма, 0) = 0
	...
	|			И ТаблицаТоваровДО.СтавкаНДС = ТаблицаТоваровСФ.СтавкаНДС
	|		ЛЕВОЕ СОЕДИНЕНИЕ вт_ТаблицаТоваровПредварительная КАК ТаблицаТоваровПредварительная
	|		ПО ТаблицаТоваровДО.ИсходныйСчетФактура = ТаблицаТоваровПредварительная.ИсходныйСчетФактура
	|			И ТаблицаТоваровДО.Номенклатура = ТаблицаТоваровПредварительная.Номенклатура
	|			И ТаблицаТоваровДО.Характеристика = ТаблицаТоваровПредварительная.Характеристика
	|			И ТаблицаТоваровДО.Серия = ТаблицаТоваровПредварительная.Серия
	|			И ТаблицаТоваровДО.НоменклатураНабора = ТаблицаТоваровПредварительная.НоменклатураНабора
	|			И ТаблицаТоваровДО.ХарактеристикаНабора = ТаблицаТоваровПредварительная.ХарактеристикаНабора
	|			И ТаблицаТоваровДО.Содержание = ТаблицаТоваровПредварительная.Содержание
	|			И ТаблицаТоваровДО.ТипЗапасов = ТаблицаТоваровПредварительная.ТипЗапасов
	|			И ТаблицаТоваровДО.КодТНВЭД = ТаблицаТоваровПредварительная.КодТНВЭД
	|			И ТаблицаТоваровДО.НомерГТД = ТаблицаТоваровПредварительная.НомерГТД
	|			И ТаблицаТоваровДО.ЕдиницаИзмерения = ТаблицаТоваровПредварительная.ЕдиницаИзмерения
	|			И ТаблицаТоваровДО.СтавкаНДС = ТаблицаТоваровПредварительная.СтавкаНДС
	|ГДЕ
	|	&ОтборПоСчетуФактуре
	|	И ТаблицаТоваровСФ.ИсходныйСчетФактура ЕСТЬ NULL
	|	И ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.Исправление, ЛОЖЬ)
	|			ТОГДА ТаблицаТоваровПредварительная.ПредыдущийСчетФактура.СчетФактураОснование
	|		ИНАЧЕ
...
Процедура РассчитатьМежценовуюРазницу(ТаблицаТовары, ПараметрыЗаполнения)
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		СтрокаТовары.СуммаНДС = Окр(СтрокаТовары.МежценоваяРазница * ЗначениеСтавкиНДС / 100, 2, РежимОкругления.Окр15как20);
		
		СтрокаТовары.МежценоваяРазницаУвеличение = СтрокаТовары.СуммаУвеличение;
		СтрокаТовары.СуммаНДСУвеличение = Окр(СтрокаТовары.МежценоваяРазницаУвеличение * ЗначениеСтавкиНДС / 100, 2, РежимОкругления.Окр15как20);
				
		СтрокаТовары.МежценоваяРазницаУменьшение = СтрокаТовары.СуммаУменьшение;
		СтрокаТовары.СуммаНДСУменьшение = Окр(СтрокаТовары.МежценоваяРазницаУменьшение * ЗначениеСтавкиНДС / 100, 2, РежимОкругления.Окр15как20);
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Если ЗаполнитьДанныеШтрихкодовДляУКДДо Тогда
		...
		РезультатПоТабличнойЧастиДоТЗ = РезультатПоТабличнойЧастиТЗ.Скопировать(,"Ссылка, Номенклатура, Характеристика, КоличествоДо");
		РезультатПоТабличнойЧастиДоТЗ.Колонки.КоличествоДо.Имя = "Количество";
		...
		Если ПараметрыПечати.Свойство("ЗаполнениеЭлектронногоДокумента") Тогда
			Если МассивШапокОснований.Количество() Тогда
				ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(МассивШапокОснований[0]);
			ИначеЕсли РезультатПоТабличнойЧастиТЗ.Количество() Тогда
				ПараметрыСканирования = ШтрихкодированиеИС.ПараметрыСканирования(РезультатПоТабличнойЧастиТЗ[0].Ссылка);
			КонецЕсли;
			МаркировкаДо = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимоеИКодыОСУ(
				МаркировкаДо,
				РезультатПоТабличнойЧастиДоТЗ,
	...
	Иначе
		...
		Если ПараметрыПечати.Свойство("ЗаполнениеЭлектронногоДокумента") Тогда
			...
			Если Выборка.Следующий() И 
				...
				Выборка = РезультатПоТабличнойЧасти.Выбрать();
				...
				КонецЦикла;
				Маркировка = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимоеИКодыОСУ(
					Маркировка, Товары, ШтрихкодированиеИС.ПараметрыСканирования(ДокументОснование));
			Иначе
				Маркировка = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимое(Маркировка);
			КонецЕсли;
		Иначе
