...
Функция ПолучитьБазуРаспределенияНДС(Дата, НачалоПериода, Организация, ДополнительныеПараметрыПолученияВыручки = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из Реквизиты Цикл
		...
		//++ Локализация
		Если СтрокаВыручки <> Неопределено
			И СтрокаВыручки.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
				СтрокаВыручкиПроизводствоСДЦ = ДанныеВыручки.ВыручкаПоНалогообложениямНДС.Найти(Перечисления.ТипыНалогообложенияНДС.ПроизводствоСДЦ, "НалогообложениеНДС");	
				Выручка = Выручка - ?(СтрокаВыручкиПроизводствоСДЦ <> Неопределено, СтрокаВыручкиПроизводствоСДЦ.Выручка, 0);
		КонецЕсли;
		//--Локализация
