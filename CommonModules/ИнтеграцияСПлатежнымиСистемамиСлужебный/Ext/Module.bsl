...
//    * ДлительностьОперацииПлатежнойСистемы - Неопределено - настройка интеграции не доступна.
//                                           - Число
//
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы() Экспорт
	
	ИнтерактивнаяЗагрузкаДоступна = ИнтеграцияСПлатежнымиСистемами.НастройкаИнтеграцияДоступна();
	
	Результат = Новый Структура();
	Результат.Вставить("НастройкаИнтеграцияДоступна"         , ИнтерактивнаяЗагрузкаДоступна);
	Результат.Вставить("ДлительностьОперацииПлатежнойСистемы", Неопределено);
	
	Если Не ИнтерактивнаяЗагрузкаДоступна Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ДлительностьОперацииПлатежнойСистемы = Константы.ДлительностьОперацииПлатежнойСистемы.Получить();
...
Процедура ПриЗаполненииХостовСервисовИнтернетПоддержки(ХостыСервисовИнтернетПоддержки) Экспорт
	...
	ХостыСервисовИнтернетПоддержки.Вставить(
		ХостСервисаОбменаДанными(),
		НСтр("ru = 'Интеграция с платежными системами'"));
...
Функция НастройкиСверкиВзаиморасчетов() Экспорт
	...
	ПлатежныеСистемы = СервисИнтеграцииССБП.ПоддержкаСверкиВзаиморасчетов();
...
Функция ПлатежныеСистемыВозврата() Экспорт
	...
	Если РезультатЗапроса.Пустой() И Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		// Обновление списка участников СБП
		// для случая, когда регламентное задание
		// не успело выполниться.
...
//
// Возвращаемое значение:
//  Массив из Строка.ПлатежныеСистемы - перечень БИК участника СБП.
//
...
//
// Возвращаемое значение:
//  Структура - данные настроек участника СБП:
...
Функция НастройкиУчастникаСБП(Идентификатор) Экспорт
	...
	Настройки.Вставить("ИНН", "");
	Настройки.Вставить("ПлатежныйАгрегатор", Ложь);
	Настройки.Вставить("КассовыеСсылки", Ложь);
	Настройки.Вставить("СверкаВзаиморасчетов", Ложь);
	Настройки.Вставить("ПодключениеКассовойСсылки", Ложь);
...
Процедура ЗаписатьНастройкиУчастниковСБП(Настройки) Экспорт
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		ЗаписьНастройкиУчастниковСБП = НаборЗаписейНастройкиУчастниковСБП.Добавить();
		ЗаписьНастройкиУчастниковСБП.Идентификатор = Настройка.memberId;
		ЗаписьНастройкиУчастниковСБП.Наименование = Настройка.memberName;
		ЗаписьНастройкиУчастниковСБП.ИНН = Настройка.inn;
		ЗаписьНастройкиУчастниковСБП.ПлатежныйАгрегатор = Настройка.paymentAggregator;
		ЗаписьНастройкиУчастниковСБП.КассовыеСсылки = Настройка.c2bCashRegisterPaymentSupported;
		ЗаписьНастройкиУчастниковСБП.СверкаВзаиморасчетов = Настройка.reconciliationOfSettlementsSupported;
		ЗаписьНастройкиУчастниковСБП.ПодключениеКассовойСсылки = Настройка.c2bCashRegisterReservedQrIdSupported;
...
Функция ПлатежныеСистемыСверкиВзаиморасчетов() Экспорт
	...
	Возврат СервисИнтеграцииССБП.ПоддержкаСверкиВзаиморасчетов();
...
Процедура ДобавитьНастройкиАутентификации(
	...
	Если ПараметрыПодсказки.ОтобразитьПодсказку Тогда
		Форма.Элементы.ДекорацияПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			Настройки.Подсказка,
			?(ЗначениеЗаполнено(ПараметрыПодсказки.АдресЛичногоКабинета),
...
Функция НастроитьПараметрыПодключения(
	...
	ЗаполнитьЗначенияСвойств(
		...
		НастройкиУчастникаСБП(ИдентификаторУчастникаСБП));
	
	НастройкиУчастникаСБП.Вставить("БИК", БИКУчастникаСБП(ИдентификаторУчастникаСБП));
