...
Функция ПолучитьКоллекцию(Коллекция, ИсходнаяКоллекция = Неопределено, НастройкиСервиса = Неопределено) Экспорт
	...
	Если Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Продажи Тогда
		Возврат ПолучитьКоллекциюПродаж(Неопределено, Истина, НастройкиСервиса);
	...
	ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы Тогда
		Возврат ПолучитьКоллекциюЗаказовКлиентовДанныеРегистра(Неопределено, НастройкиСервиса);
	ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Промо Тогда
		Возврат ПолучитьКоллекциюПромоАкций(Неопределено, НастройкиСервиса);
	ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж Тогда
		СценарийКонтрольныхПланов = НастройкиСервиса.СценарийПрогнозированияКонтрольныхПланов;
		Если ЗначениеЗаполнено(СценарийКонтрольныхПланов) Тогда
			Возврат ПолучитьКоллекциюКонтрольныхПлановПродаж(СценарийКонтрольныхПланов, Неопределено, НастройкиСервиса);
		Иначе
			ОписаниеПричиныОшибки = Новый Структура();
			ОписаниеПричиныОшибки.Вставить("ПричинаОшибки", "НеУказанСценарийПрогнозированияКонтрольныхПланов");
			Возврат ОписаниеПричиныОшибки;
		КонецЕсли;
	...
	ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь Тогда
		...
		Если ЗначениеЗаполнено(Календарь) Тогда
			Возврат ПолучитьКоллекциюКалендарь(Календарь);
		Иначе
			ОписаниеПричиныОшибки = Новый Структура();
			ОписаниеПричиныОшибки.Вставить("ПричинаОшибки", "НеУказанКалендарьПредприятия");
			Возврат ОписаниеПричиныОшибки;
		КонецЕсли;
	Иначе
...
Функция ПолучитьСводкуПоКоллекции(Коллекция, НастройкиСервиса = Неопределено) Экспорт
	...
	Если Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Продажи
		...
		ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж Тогда
			СценарийКонтрольныхПланов = НастройкиСервиса.СценарийПрогнозированияКонтрольныхПланов;
			Если ЗначениеЗаполнено(СценарийКонтрольныхПланов) Тогда
				УказанСценарийКонтрольныхПланов = Истина;
				ДатыОстатков                    = ПолучитьДатыНачалаОкончанияКонтрольныхПлановПродаж(СценарийКонтрольныхПланов);
				ДатаАктуальности                = ДатыОстатков.ДатаНачалаПродаж;
			Иначе
				УказанСценарийКонтрольныхПланов = Ложь;
				ДатаАктуальности                = Дата(1, 1, 1);
			КонецЕсли;
		ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь Тогда
			Календарь = Константы.ОсновнойКалендарьПредприятия.Получить();
			Если ЗначениеЗаполнено(Календарь) Тогда
				УказанКалендарь     = Истина;
				ДатаНачалаКалендаря = ПолучитьДатыНачалаОкончанияКалендаря(Календарь);
				ДатаАктуальности    = ДатаНачалаКалендаря;
			Иначе
				УказанКалендарь     = Ложь;
				ДатаАктуальности    = Дата(1, 1, 1);
			КонецЕсли;
		ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы Тогда
		...
		Если Не ЗначениеЗаполнено(ДатаАктуальности) Тогда
			Ответ = ОписаниеОтветаСводкиПоКоллекции();
			Если Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь
				И Не УказанКалендарь Тогда
				Ответ.Вставить("ПричинаОшибки", "НеУказанКалендарьПредприятия");
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж
				И Не УказанСценарийКонтрольныхПланов Тогда
				Ответ.Вставить("ПричинаОшибки", "НеУказанСценарийПрогнозированияКонтрольныхПланов");
			КонецЕсли;
			Возврат Ответ;
		КонецЕсли;
		...
		Пока СервисПрогнозирования.ЕстьНевыгруженныйПериод(Коллекция, ДатаАктуальности) Цикл
			...
			Если Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Продажи Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюПродаж(ДиапазонДатДляВыгрузки, Ложь, НастройкиСервиса);
				КоллекцияДанных = ПорцияВыгрузки.Коллекция;
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Остатки Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюОстатков(ДиапазонДатДляВыгрузки);
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Промо Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюПромоАкций(ДиапазонДатДляВыгрузки, НастройкиСервиса);
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюКонтрольныхПлановПродаж(СценарийКонтрольныхПланов, ДиапазонДатДляВыгрузки, Неопределено);
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюКалендарь(Календарь, ДиапазонДатДляВыгрузки);
			ИначеЕсли Коллекция = Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы Тогда
				ПорцияВыгрузки = ПолучитьКоллекциюЗаказовКлиентовДанныеРегистра(ДиапазонДатДляВыгрузки, НастройкиСервиса);
			КонецЕсли;
			...
			Если Коллекция <> Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь Тогда
...
//  ПрогнозПоОптимальнойМодели - Булево - Признак того, был ли получен прогноз по оптимальной или выбранной модели
//  ИдОбучения - Строка - 
//  КачествоМодели - Массив из Структура - 
Процедура ЗаписатьПрогнозПродаж(СтруктураОтвета, ВидПлана, ПрогнозПоОптимальнойМодели, ИдОбучения, КачествоМодели) Экспорт
	...
	Для Каждого ТекущийПрогноз Из СтруктураОтвета Цикл
		Если ПрогнозПоОптимальнойМодели Тогда
...
//  ИдОбучения - Строка - 
//  КачествоМодели - Массив из Структура - 
Процедура ЗаписатьПрогнозПродажПоКатегориям(СтруктураОтвета, ВидПлана, ПрогнозПоОптимальнойМодели, ИдОбучения, КачествоМодели) Экспорт
...
// 
// Параметры:
//  СценарийКонтрольныхПланов - СправочникСсылка.СценарииТоварногоПланирования - Сценарий контрольных планов продаж
// 
// Возвращаемое значение:
//  Структура:
//   * ДатаНачалаПродаж - Дата -
//   * ДатаОкончанияПродаж - Дата -
Функция ПолучитьДатыНачалаОкончанияКонтрольныхПлановПродаж(СценарийКонтрольныхПланов)
	...
	|ПОМЕСТИТЬ ВтДаты
	|ИЗ
	|	РегистрНакопления.ПланыПродаж.Обороты(, , ДЕНЬ, Сценарий = &СценарийКонтрольныхПланов) КАК Продажи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("СценарийКонтрольныхПланов", СценарийКонтрольныхПланов);
...
// 
// Параметры:
//  Календарь - СправочникСсылка.Календари - Календарь предприятия
// 
// Возвращаемое значение:
//  Структура:
//   * ДатаНачалаКалендаря - Дата -
Функция ПолучитьДатыНачалаОкончанияКалендаря(Календарь)
...
// Возвращаемое значение:
//  см. ОписаниеОтветаПорцииПериодическойКоллекции.
Функция ПолучитьКоллекциюПродаж(ДиапазонДатДляВыгрузки = Неопределено, ПолучитьДополнительныеРеквизиты = Истина, НастройкиСервиса = Неопределено)
...
Функция ПолучитьКоллекциюКатегорийИсходную()
	...
	|	НЕ Номенклатура.ПометкаУдаления
	|	И НЕ Номенклатура.ЭтоГруппа
	|	И НЕ Номенклатура.ТоварнаяКатегория.Ссылка ЕСТЬ NULL
	|	И (НЕ &УстановленОтборПоНоменклатуре
	|		ИЛИ
	|		Номенклатура.Ссылка В ИЕРАРХИИ(&ОтборНоменклатуры))";
...
Функция ПолучитьКоллекциюЗаказовКлиентовДанныеРегистра(ДиапазонДатДляВыгрузки = Неопределено, НастройкиСервиса = Неопределено)
	
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ДанныеЗаказа.Период, ДЕНЬ) КАК Дата,
	|	СУММА(ДанныеЗаказа.ЗаказаноПриход) КАК КоличествоЗаказанное,
	|	СУММА(ДанныеЗаказа.ЗаказаноРасход) КАК КоличествоПроданное,
	|	СУММА(ДанныеЗаказа.СуммаПриход) КАК Сумма,
	|	СУММА(ВЫБОР
	|		КОГДА
	|			ДанныеЗаказа.ЗаказаноПриход <> 0 И ДанныеЗаказа.СуммаПриход <> 0
	|		ТОГДА
	|			ВЫРАЗИТЬ(ДанныеЗаказа.СуммаПриход / ДанныеЗаказа.ЗаказаноПриход КАК ЧИСЛО(31,2))
	|		ИНАЧЕ
	|			0
	...
	|ВЫБРАТЬ
	|	Набор.Дата КАК Дата,
	|	СУММА(Набор.КоличествоЗаказанное) КАК Количество,
	...
	|	ВЫБРАТЬ
	|		Набор.Дата КАК Дата,
	|		Набор.КоличествоЗаказанное КАК КоличествоЗаказанное,
	|		Набор.КоличествоПроданное КАК КоличествоПроданное,
	|		Набор.Сумма КАК Сумма,
	...
	Если ДиапазонДатДляВыгрузки = Неопределено Тогда
		...
		ДиапазонДатДляВыгрузки = СервисПрогнозирования.ДиапазонДатДляВыгрузки(
			Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы, ДатыЗаказов.ДатаНачалаЗаказов);
	КонецЕсли;
	...
	Если НастройкиСервиса = Неопределено Тогда
		НастройкиСервиса = СервисПрогнозирования.ПолучитьНастройкиСервиса();
	КонецЕсли;
	ИнфоОКоллекции = НастройкиСервиса.Коллекции[Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы];
	Если ИнфоОКоллекции.ВложенноеОписание["КодКлиента"].Выгружать Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КодКлиента", "Набор.КодКлиента");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КодКлиента", "&ПустаяСтрока");
...
Функция ПолучитьКоллекциюПромоАкций(ДиапазонДатДляВыгрузки = Неопределено, НастройкиСервиса = Неопределено)
	...
	|
	|ВЫБРАТЬ
	|	Набор.ДатаНачала КАК ДатаНачала,
	|	Набор.ДатаОкончания КАК ДатаОкончания,
	...
	Если Ответ.КоличествоДанных > 0 Тогда
		...
		Если Ответ.ДатаПервойЗаписи = Неопределено Тогда
			Ответ.ДатаПервойЗаписи = Дата(1,1,1);
		КонецЕсли;
		Если Ответ.ДатаПоследнейЗаписи = Неопределено Тогда
			Ответ.ДатаПоследнейЗаписи = Дата(1,1,1);
		КонецЕсли;
...
Функция ПолучитьКоллекциюКонтрольныхПлановПродаж(СценарийКонтрольныхПланов, ДиапазонДатДляВыгрузки = Неопределено, НастройкиСервиса = Неопределено)
	
	Запрос = Новый Запрос();
	Запрос.Текст =
	"ВЫБРАТЬ
	|	&СценарийКонтрольныхПланов КАК Сценарий,
	|	Продажи.КоличествоОборот КАК Количество,
	|	Продажи.СуммаОборот КАК Сумма,
	|	ВЫБОР
	|		КОГДА Продажи.КоличествоОборот <> 0
	...
	|ПОМЕСТИТЬ ВТОстаткиПланов
	|ИЗ
	|	РегистрНакопления.ПланыПродаж.Обороты(
	|			&ДатаНачала,
	|			&ДатаОкончания,
	|			Регистратор,
	|			(Сценарий = &СценарийКонтрольныхПланов
	|				И (НЕ &УстановленОтборПоНоменклатуре
	|					ИЛИ Номенклатура В ИЕРАРХИИ (&ОтборНоменклатуры))
	|				И (НЕ &УстановленОтборПоПартнерам
	|					ИЛИ Партнер В ИЕРАРХИИ (&ОтборПартнеров))
	...
	|	Набор.КодСклада КАК КодСклада,
	|	Набор.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	Набор.Периодичность КАК Периодичность,
	|	Набор.ДатаПланирования КАК ДатаПланирования,
	|	Набор.ДатаСозданияПланаПродаж КАК ДатаСозданияПланаПродаж,
	|	Набор.ДатаНачалаПериодаПланирования КАК ДатаНачалаПериодаПланирования,
	|	Набор.ДатаОкончанияПериодаПланирования КАК ДатаОкончанияПериодаПланирования
	|ИЗ
	|	ВТОстаткиПланов КАК Набор
	|СГРУППИРОВАТЬ ПО
	|	Набор.Сценарий,
	|	Набор.КодТовара,
	|	&КодХарактеристики,
	...
	|	Набор.ХозяйственнаяОперация,
	|	Набор.Периодичность,
	|	Набор.ДатаПланирования,
	|	Набор.ДатаСозданияПланаПродаж,
	|	Набор.ДатаНачалаПериодаПланирования,
	|	Набор.ДатаОкончанияПериодаПланирования
	|УПОРЯДОЧИТЬ ПО
	|	Набор.ДатаПланирования,
	|	Набор.ДатаСозданияПланаПродаж,
	|	Набор.ДатаНачалаПериодаПланирования,
	...
	Если ДиапазонДатДляВыгрузки = Неопределено Тогда
		ДатаНачалаПродаж = ПолучитьДатыНачалаОкончанияКонтрольныхПлановПродаж(СценарийКонтрольныхПланов).ДатаНачалаПродаж;
		ДиапазонДатДляВыгрузки = СервисПрогнозирования.ДиапазонДатДляВыгрузки(
			Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж, ДатаНачалаПродаж);
	КонецЕсли;
	...
	Если Ответ.КоличествоДанных > 0 Тогда
		Ответ.ДатаПервойЗаписи = Результат[0].ДатаПланирования;
		Ответ.ДатаПоследнейЗаписи = Результат[Ответ.КоличествоДанных - 1].ДатаПланирования;
	КонецЕсли;
...
Функция ПолучитьКоллекциюКалендарь(Календарь, ДиапазонДатДляВыгрузки = Неопределено)
	...
	Если ДиапазонДатДляВыгрузки = Неопределено Тогда
		ДатаНачалаКалендаря = ПолучитьДатыНачалаОкончанияКалендаря(Календарь);
		ДиапазонДатДляВыгрузки = СервисПрогнозирования.ДиапазонДатДляВыгрузки(
			Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь, ДатаНачалаКалендаря);
	КонецЕсли;
...
Функция ПолучитьОписаниеРеквизитовКоллекцииПлановПродаж()
	...
	Описание.Вставить("Сценарий",
		ОписаниеВыгружаемогоЭлементаДанных("scenario", Истина, Истина, "Сценарий", НСтр("ru='Сценарий'")));
	...
	Описание.Вставить("ДатаСозданияПланаПродаж",
		ОписаниеВыгружаемогоЭлементаДанных("dt_plan_input", Истина, Истина, "ДатаСозданияПланаПродаж", НСтр("ru='Дата формирования плана продаж'")));
	...
	Описание.Вставить("ДатаПланирования",
		ОписаниеВыгружаемогоЭлементаДанных("dt_plan", Истина, Истина, "ДатаПланирования", НСтр("ru='Дата планирования'")));
	Описание.Вставить("КодТовара",
		ОписаниеВыгружаемогоЭлементаДанных("item_id", Истина, Истина, "КодТовара", НСтр("ru='Код товара'")));
	Описание.Вставить("Количество",
		ОписаниеВыгружаемогоЭлементаДанных("quantity", Истина, Истина, "Количество", НСтр("ru='Количество'")));
...
Функция ПолучитьОписаниеРеквизитовКоллекцииКалендарь()
	...
	// В будущем календарь станет встроенным источником данных, для которого не происходит выгрузка данных на сервер.
	Описание.Вставить("Дата",
		ОписаниеВыгружаемогоЭлементаДанных("dt_holiday", Истина, Истина, "Дата", НСтр("ru='Дата'")));
	Описание.Вставить("ЭтоПраздник",
		ОписаниеВыгружаемогоЭлементаДанных("holiday", Истина, Истина, "ЭтоПраздник", НСтр("ru='День является праздником'")));
...
Процедура ЗаполнитьДанныеТекущегоПрогнозаСВыбраннойМоделью(ТекущийПрогноз, ПрогнозПоТоварам, ДанныеДляЗагрузки,
	...
	Если ПрогнозПоТоварам Тогда
		...
		Для Каждого КлючЗначение Из ТекущийПрогноз Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "model" Тогда
				Продолжить; // Не применимо. Присутствует при запросе predictions_full.
			...
			ИначеЕсли КлючЗначение.Ключ = "prediction" Тогда
