...
Функция ОбработатьСписокДокументов(МассивДанных, ЕстьОшибки, СообщениеОбОшибке)
	...
	Для Каждого Смена Из МассивДанных Цикл
		...
		Для Каждого Чек Из Смена.ДанныеЧеков Цикл
			...
			Для Каждого Товар Из Чек.Товары Цикл // Структура
				...
				Если ЗначениеЗаполнено(Товар.КодПредметаРасчета) Тогда
					ТоварМераКоличестваПредметаРасчета = Товар.МераКоличестваПредметаРасчета; // Структура
					ТоварККМ.МераКоличестваПредметаРасчета.МераКоличестваПредметаРасчета = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.МераКоличестваПредметаРасчетаККТПоКоду(ТоварМераКоличестваПредметаРасчета.Идентификатор);
					ТоварККМ.МераКоличестваПредметаРасчета.Значение = Товар.МераКоличестваПредметаРасчета.Значение;
				КонецЕсли;
				
				ТоварДанныеАгента = Товар.ДанныеАгента; // Структура
				Если ЗначениеЗаполнено(ТоварДанныеАгента) Тогда
					ТоварККМ.ДанныеАгента.ИНН = ТоварДанныеАгента.ИНН;
					ТоварККМ.ДанныеАгента.Наименование = ТоварДанныеАгента.Наименование;
				КонецЕсли;
				
				ТоварДанныеПоставщика = Товар.ДанныеПоставщика; // Структура
				Если ЗначениеЗаполнено(ТоварДанныеПоставщика) Тогда
					ТоварККМ.ДанныеПоставщика.ИНН = ТоварДанныеПоставщика.ИНН;
					ТоварККМ.ДанныеПоставщика.Наименование = ТоварДанныеПоставщика.Наименование;
				КонецЕсли;
