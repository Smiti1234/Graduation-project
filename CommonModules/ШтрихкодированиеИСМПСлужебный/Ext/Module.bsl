...
Процедура ЗаполнитьДанныеИдентификатораРозничнойПродажиПоОбязательнымТоварнымГруппам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если Не ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП
		И (Не ПараметрыСканирования.ЗапрашиватьДанныеСервисаИСМП
		Или Не ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеКМ              = Новый Массив;
	ПолучатьИдентификатор = ИнтеграцияИСМПКлиентСерверПовтИсп.ПроверкаТоварныхГруппПриРозничнойПродажеГИСМТ();
	...
	Результат = ИнтерфейсИСМП.СокращеннаяИнформацияПоКМПриРозничнойПродаже(ДанныеКМ, ПараметрыСканирования.Организация, Не ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП);
	...
	Для Каждого СтрокаДанныхПоШтрихкодам Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		СтрокаДанныхИзГИСМТ    = Результат.СтатусыКодовМаркировки.Получить(СтрокаДанныхПоШтрихкодам);
		
		Если СтрокаДанныхИзГИСМТ = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Для Каждого КлючИЗначение Из СтрокаДанныхИзГИСМТ Цикл
		...
		НаличиеСерыхКодов = ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(СтрокаДанных.ВидПродукции)
			И СтрокаДанныхИзГИСМТ.Свойство("ВСеройЗоне")
			И СтрокаДанныхИзГИСМТ.ВСеройЗоне;
		...
		Если Не СтрокаДанныхИзГИСМТ.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаПодчиненнойУпаковки Из СтрокаДанныхПоШтрихкодам.СтрокаДерева.Строки Цикл
			...
			Если ПолучатьИдентификатор Тогда
				...
				СтруктураЗаполнения.Вставить("РазрешенаПродажа",                             Неопределено);
				
				СтруктураЗаполнения.РазрешенаПродажа = Результат.РазрешенаПродажа;
				
			КонецЕсли;
			
			ЗаполнитьЗначенияСвойств(СтруктураЗаполнения, СтрокаДанныхИзГИСМТ);
...
Функция ШтрихкодыУпаковокИзДокумента(Знач ДокументСсылка, Знач ПараметрыСканирования) Экспорт
	...
	Если ИнтеграцияИСМПКлиентСерверПовтИсп.ПроверкаТоварныхГруппПриРозничнойПродажеГИСМТ() Тогда
...
Процедура УстановитьПризнакОтказаОтПроверкиКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей = ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(СтрокаДанных.ВидПродукции,
		...
		Если Не СтрокаДанных.ЕстьВПулеКодовМаркировки И Не ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей Тогда
			НеобходимоДелатьЗапрос = Истина;
			Прервать;
		КонецЕсли;
		...
		Иначе
			НеобходимоДелатьЗапрос = Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(
...
Процедура ЗаполнитьПараметрыДополненияВложенныхШтрихкодов(ПараметрыДополнения, ШтрихкодыУпаковок, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ШтрихкодыУпаковок Цикл
		...
		Если ЕстьВидПродукцииВСтроке Тогда
			Если Не ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(СтрокаТаблицы, ПараметрыСканирования,, Истина)
				И Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(СтрокаТаблицы.ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП, ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ) Тогда
				Продолжить;
			КонецЕсли;
		Иначе
		...
		Если ЗначениеЗаполнено(ЭлементДополнения.РазрешительныйРежимИдентификаторЗапросаГИСМТ) Тогда
...
Процедура ПроверитьНаОшибкиТаблицуДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольСтатусов")
			...
			Если СтрокаДанных.РазрешенаПродажа И СтрокаДанных.Статус = Перечисления.СтатусыКодовМаркировкиМОТП.Нанесен
				...
				Если СтрокаДанных.Производитель = ОжидаемыйИННВладельца Тогда
					ОтсутствуетДопустимыйСтатус = Ложь;
				КонецЕсли;
			ИначеЕсли СтрокаДанных.РазрешенаПродажа И СтрДлина(СтрокаДанных.СоставКодаМаркировки.СерийныйНомер) = 8
				И ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(СтрокаДанных.ВидПродукции)
				И СтрНачинаетсяС(СтрокаДанных.СоставКодаМаркировки.СерийныйНомер, "2") Тогда
