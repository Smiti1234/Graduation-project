...
//  Организация - СправочникСсылка.Организации - Организация
//  ТаймаутНаПродажу - Булево - использовать таймаут в 1,5 секунды на розничную продажу. Если за таймаут не был получен ответ,
//   продажа разрешена
// 
// Возвращаемое значение:
...
// * ТребуетсяОбновлениеКлючаСессии - Булево - признак необходимости обновления ключа сессии
// * РезультатОтправкиЗапроса - (См. ИнтерфейсМОТПСлужебный.ОбработатьРезультатОтправкиHTTPЗапросаКакJSON).
// * ПодтверждениеВозможностиПродажи - Булево - признак возможности реализации КМ, полученный от ГИС МТ
// * СтатусыКодовМаркировки - Соответствие Из КлючИЗначение - Статусы кодов маркировки:
//		** Ключ - Строка - Код маркировки.
//		** Значение - (См. ИнтерфейсИСМПСлужебный.ПараметрыКодаМаркировки).
// * РазрешенаПродажа - Булево - признак разрешения товара к продаже. Либо по результатам проверки ГИС МТ, либо по таймауту запроса
// * ТекстОшибки - Строка - текст ошибки, полученной от ГИС МТ
Функция СокращеннаяИнформацияПоКМПриРозничнойПродаже(ДанныеПроверкиКМ, Организация = Неопределено, ТаймаутНаПродажу = Истина) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ТребуетсяОбновлениеКлючаСессии",  КлючСессии = Неопределено);
	ВозвращаемоеЗначение.Вставить("РезультатОтправкиЗапроса",        Неопределено);
	ВозвращаемоеЗначение.Вставить("ПодтверждениеВозможностиПродажи", Ложь);
	ВозвращаемоеЗначение.Вставить("СтатусыКодовМаркировки",          Новый Соответствие);
	ВозвращаемоеЗначение.Вставить("РазрешенаПродажа",                Ложь);
	ВозвращаемоеЗначение.Вставить("ТекстОшибки",                     "");
	...
	// таймаут на получение идентификатора ГИС МТ - 1,5 секунды. Если за 1,5 секунды ответ не был получен,
	// продажа разрешена
	Если ТаймаутНаПродажу Тогда
		ПараметрыЗапросов.Таймаут = 1.5;
	КонецЕсли;
	...
	Для Каждого КодМаркировки Из МассивКМ Цикл
		...
		ПараметрыЗапроса.Вставить("inn",   ИнтеграцияИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(Организация).ИНН);
				...
				Иначе
					...
					Если ТипЗнч(ДанныеПоКодамМаркировки) = Тип("Массив") Тогда
						...
						Для Каждого ЭлементДанных Из ДанныеПоКодамМаркировки Цикл
							...
							Если ЭлементДанных.Получить("printView") <> Неопределено Тогда
							
								КодМаркировки   = ЭлементДанных["printView"];
								ДанныеШтрихкода = СоответствиеПолногоКодаКМ.Получить(КодМаркировки);
								
								Если ДанныеШтрихкода = Неопределено Тогда
									КодМаркировки   = ЭлементДанных["cis"];
									ДанныеШтрихкода = СоответствиеПолногоКодаКМ.Получить(КодМаркировки);
								КонецЕсли;
							...
							Если ЗначениеЗаполнено(КодОшибки) Тогда
								...
								Иначе
									ТекстОшибки = НСтр("ru = 'Неклассифицированная ошибка отправки запроса по КМ в ГИС МТ.'");
								КонецЕсли;
							...
							Иначе
								...
								Если ПараметрыКодаМаркировки.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен
									Или ПараметрыКодаМаркировки.Статус = Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен Тогда
								...
								ВозвращаемоеЗначение.СтатусыКодовМаркировки.Вставить(ДанныеШтрихкода, ПараметрыКодаМаркировки);
								
								Если ЕстьОшибкиОбработки Тогда
									ВозвращаемоеЗначение.РазрешенаПродажа = Ложь;
								Иначе
									
									ВозвращаемоеЗначение.РазрешенаПродажа = Истина;
		...
		Иначе
			...
			// ответ не получен, либо таймаут, либо иные проблемы, продажу разрешить
			ВозвращаемоеЗначение.РазрешенаПродажа = Истина;
