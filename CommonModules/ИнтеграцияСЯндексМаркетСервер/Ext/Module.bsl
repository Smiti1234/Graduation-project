		...
		Для каждого ЭлементКоллекции Из Магазины.campaigns Цикл
			...
			Если СтрокаТаблицыЗначений = Неопределено Тогда
				Попытка
					...
					НоваяУчетнаяЗапись.СхемаРаботы                       = ОпределитьСхемуРаботы(ЭлементКоллекции.placementType);
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.Наименование)
					 ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.ИдентификаторАккаунта)
					 ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.ИдентификаторКабинета)
					 ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.СхемаРаботы) Тогда
					Попытка
						УчетнаяЗапись = СтрокаТаблицыЗначений.Ссылка.ПолучитьОбъект();
						Если НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.Наименование) Тогда
							УчетнаяЗапись.Наименование = ЭлементКоллекции.domain;
						КонецЕсли;
						Если НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.ИдентификаторАккаунта) Тогда
							УчетнаяЗапись.ИдентификаторАккаунта = ИдентификаторАккаунта;
						КонецЕсли;
						Если НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.ИдентификаторКабинета) Тогда
							УчетнаяЗапись.ИдентификаторКабинета = Формат(ЭлементКоллекции.business.id, "ЧН=; ЧГ=0");
						КонецЕсли;
						Если НЕ ЗначениеЗаполнено(СтрокаТаблицыЗначений.СхемаРаботы) Тогда
							УчетнаяЗапись.СхемаРаботы = ОпределитьСхемуРаботы(ЭлементКоллекции.placementType);
						КонецЕсли;
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Если Ошибки <> Неопределено Тогда
				Для каждого ЭлементКоллекции Из Ошибки Цикл
					...
					Запись.КодОшибки      = ?( ЗначениеЗаполнено(Запись.КодОшибки), "; ", "") + ЭлементКоллекции.code; 
					Запись.ОписаниеОшибки = ?( ЗначениеЗаполнено(Запись.ОписаниеОшибки), "; ", "") + ?(ЗначениеЗаполнено(ПереводОшибки), ПереводОшибки, ЭлементКоллекции.message);
				КонецЦикла;
			КонецЕсли;
...
Процедура ВыгрузитьОстаткиТоваровВСервис(ДанныеУчетнойЗаписи, Отказ = Ложь) 
	...
	Запрос.Текст = 
		...
		|		ПО СтатусыПубликацииТоваровЯндексМаркет.УчетнаяЗапись = СоответствияОбъектовМаркетплейсов.УчетнаяЗаписьМаркетплейса
		...
		|	И (ВЫРАЗИТЬ(СоответствияОбъектовМаркетплейсов.Объект1С КАК Справочник.Склады)) <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|	И ЕСТЬNULL(СоответствияОбъектовМаркетплейсов.ИдентификаторОбъектаМаркетплейса, """") <> """"
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Характеристика,
	...
	ОстаткиТоваров = ПолучитьРазницуОстатковТоваровДляВыгрузки(ДанныеУчетнойЗаписи.УчетнаяЗапись, МенеджерВТ); 
	
	ОценкаПроизводительности.ЗакончитьЗамерДлительнойОперации(Замер, ОстаткиТоваров.Количество() / 10000);

	Если ОстаткиТоваров.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Порция = ОстаткиТоваров.СкопироватьКолонки();
	skus   = Новый Массив;
	...
	Для каждого СтрокаТаблицыЗначений Из ОстаткиТоваров Цикл
		...
		StockItemDTO.count     = СтрокаТаблицыЗначений.count;
		...
		StockDTO.sku         = СтрокаТаблицыЗначений.sku;
		StockDTO.warehouseId = СтрокаТаблицыЗначений.warehouseId;
...
Функция ПолучитьРазницуОстатковТоваровДляВыгрузки(УчетнаяЗапись, МенеджерВТ) 
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(Запасы.ВНаличииОстаток - Запасы.РезервироватьНаСкладеОстаток - Запасы.РезервироватьПоМереПоступленияОстаток, 0) КАК Остаток,
		...
		|			И Запасы.Номенклатура = ОстаткиТоваровМаркетплейсов.Номенклатура
		|			И Запасы.Характеристика = ОстаткиТоваровМаркетплейсов.Характеристика
		|ГДЕ
		|	ЕСТЬNULL(Запасы.ВНаличииОстаток - Запасы.РезервироватьНаСкладеОстаток - Запасы.РезервироватьПоМереПоступленияОстаток, 0) <> ЕСТЬNULL(ОстаткиТоваровМаркетплейсов.Количество, 0)
		|	И Запасы.ВНаличииОстаток - Запасы.РезервироватьНаСкладеОстаток - Запасы.РезервироватьПоМереПоступленияОстаток >= 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Характеристика,
		...
		|			ТОГДА ЕСТЬNULL(Остатки.Остаток, 0)
		|		ИНАЧЕ ВЫРАЗИТЬ(ЕСТЬNULL(Остатки.Остаток, 0) / &ТекстЗапросаКоэффициентУпаковки КАК ЧИСЛО(15, 0))
		|	КОНЕЦ КАК count
		|ИЗ
		|	ВтТовары КАК Товары
...
Процедура УстановитьЦеныВСервисе(ДанныеУчетнойЗаписи, ТаблицаТоваров = Неопределено, Отказ = Ложь)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		...
		|			ИНАЧЕ 1
		|		КОНЕЦ * ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ВЫБОР
...
//     * Характеристика              - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика товарной позиции;
//     * Упаковка                    - СправочникСсылка.УпаковкиЕдиницыИзмерения - упаковка товарной позиции;
//     * ИдентификаторТовараПлощадки - Строка - идентификатор товарной позиции;
