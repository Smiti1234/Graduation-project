...
Процедура УстановитьВидимостьПодбораПоОстаткамРасчетов(ХозяйственнаяОперация, Кнопка) Экспорт
	...
	МассивОпераций.Добавить(Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
Процедура УстановитьУсловноеОформлениеПартнера(Форма) Экспорт
	...
	// Для списания денежных средств и заявки
...
Процедура УстановитьПараметрыВыбораПартнера(Объект, ЭлементПартнер) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому 
...
Процедура УстановитьПараметрыВыбораЗаявки(Объект, ЭлементЗаявка) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
		ИЛИ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому Тогда
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&ТекущаяДата, ) КАК КурсыДоговоров
...
Процедура ЗаполнитьПоОстаткамЗаявокНаРасходованиеДенежныхСредств(ДанныеОтбора, ТабличнаяЧасть) Экспорт
	...
	ИначеЕсли ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию
		...
		КонецЕсли;
	ИначеЕсли ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаСамозанятому Тогда
		Получатель = ПолучательПоЗаявкамНаОплатуСамозанятым(ДанныеОтбора);
	Иначе
...
Функция ДокументыКСозданию(СтрокиГрафика, ТипДокумента, Группировать = Истина) Экспорт
	...
	|			ЛОЖЬ
	|		КОГДА ГрафикПлатежей.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаСамозанятому) 
		...
		Для Каждого СтрокаКоэффициента Из ТаблицаСуммПоКоэффициентам Цикл
			...
			Если ИтогСумма <> 0 Тогда
				...
				Для Каждого СтрокаСуммы Из СтрокиСумм Цикл
					...
					Сумма = Окр(СуммаПоКоэффициенту * СтрокаТаблицы.Сумма / ИтогСумма, 2, 1);
...
Функция ОпределитьПлатежСКонвертацией(Объект) Экспорт
	...
	Если (ЗначениеЗаполнено(Объект.БанковскийСчетКонтрагента) Или ЗначениеЗаполнено(Объект.БанковскийСчетПолучатель))
		И (ЗначениеЗаполнено(Объект.БанковскийСчет) Или ЗначениеЗаполнено(Объект.Валюта)) Тогда
		
		Если ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
			ВалютаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчет, "ВалютаДенежныхСредств");
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.БанковскийСчетПолучатель) Тогда
			ВалютаПолучателя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.БанковскийСчетПолучатель, "ВалютаДенежныхСредств");
		КонецЕсли;
		
		Результат = ВалютаПолучателя <> ВалютаДокумента;
	КонецЕсли;
...
Функция ОтборВыплатаСамозанятому() Экспорт
	...
	ФинансоваяОтчетностьСервер.НовыйОтбор(
		ГруппаИ,
		"ХозяйственнаяОперация",
		Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
Процедура ЗаполнитьДанныеОбъектаРасчетовРасшифровкиПлатежа(ДокументОбъект, ИдентификаторСтроки = Неопределено) Экспорт
	...
	Если ИдентификаторСтроки = Неопределено Тогда
		...
		Для Каждого СтрокаТаблицы Из ДокументОбъект.РасшифровкаПлатежа Цикл
			...
			ЗаполнитьСтрокуРасшифровкиДоговором(ДокументОбъект, СтрокаТаблицы);
...
Функция НераспределенныйОстатокПлатежа(Объект, Ссылка = Неопределено) Экспорт
	...
	ОперацииВзаиморасчетов.Добавить(Перечисления.ХозяйственныеОперации.ОплатаСамозанятому);
...
//
Процедура ЗаполнитьСтрокуРасшифровкиДоговором(Объект, СтрокаТаблицы)
