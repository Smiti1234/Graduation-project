			...
			Пока ЧтениеXML.Прочитать() Цикл  
				Если ЧтениеXML.Имя = "Record" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					Если Не ПустаяСтрока(ШтрихкодBase64) Тогда
						...
						Если СтрНайти(Штрихкод, МенеджерОборудованияМаркировкаКлиентСервер.РазделительGS1()) > 0 Тогда
							ШтриховойКодТовара = МенеджерОборудованияМаркировкаКлиентСервер.РазобратьШтриховойКодТовараСлужебная(Штрихкод);
					...
					Если Не ПустаяСтрока(ШтрихкодУпаковкиBase64) Тогда
						...
						Если СтрНайти(ШтрихкодУпаковки, МенеджерОборудованияМаркировкаКлиентСервер.РазделительGS1()) > 0 Тогда
							ШтриховойКодТовара = МенеджерОборудованияМаркировкаКлиентСервер.РазобратьШтриховойКодТовараСлужебная(Штрихкод);
							Если ШтриховойКодТовара.Разобран Тогда
