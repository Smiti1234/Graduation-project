...
Процедура ПараметрыМетодаИзЗапроса(Запрос, Результат) Экспорт
	...
	ПродажаЗначение = Запрос["ПараметрыЗапроса"].Получить("TypeOperationSale");
	Если ПродажаЗначение = Неопределено Тогда
		ПараметрыМетода.Вставить("Продажа"				, ПродажаЗначение);
	Иначе
		ПараметрыМетода.Вставить("Продажа"				, Булево(ПродажаЗначение));
	КонецЕсли;
