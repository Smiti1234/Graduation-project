...
Функция ТипОплатыПоДокументу(ДокументСсылка) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   |	ВзаимозачетЗадолженности.ВидОперации КАК ВидОперации,
				   ...
				   |	ВзаимозачетЗадолженности.КонтрагентДебитор.ЮрФизЛицо КАК КонтрагентДебиторЮрФизЛицо,
				   |	ВзаимозачетЗадолженности.КонтрагентКредитор.ЮрФизЛицо КАК КонтрагентКредиторЮрФизЛицо,
				   |	ВзаимозачетЗадолженностиКредиторскаяЗадолженность.НомерСтроки КАК НомерСтроки,
				   |	ВЫБОР
				   |		КОГДА ВзаимозачетЗадолженностиКредиторскаяЗадолженность.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
				   |			ТОГДА ЛОЖЬ
				   |		ИНАЧЕ ВЫБОР
				   |				КОГДА ВзаимозачетЗадолженностиКредиторскаяЗадолженность.ОбъектРасчетов.Объект ССЫЛКА Справочник.ПодарочныеСертификаты
				   |					ТОГДА ИСТИНА
				   |				ИНАЧЕ ЛОЖЬ
				   |			КОНЕЦ
				   |	КОНЕЦ КАК ТипОбъектаПодарочныйСертификат
				   |ИЗ
				   |	Документ.ВзаимозачетЗадолженности КАК ВзаимозачетЗадолженности
				   |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВзаимозачетЗадолженности.КредиторскаяЗадолженность КАК ВзаимозачетЗадолженностиКредиторскаяЗадолженность
				   |		ПО (ВзаимозачетЗадолженностиКредиторскаяЗадолженность.Ссылка = ВзаимозачетЗадолженности.Ссылка)
				   |ГДЕ
				   |	ВзаимозачетЗадолженности.Ссылка = &Ссылка
				   |	И НЕ ВзаимозачетЗадолженностиКредиторскаяЗадолженность.Ссылка ЕСТЬ NULL
				   |
				   |СГРУППИРОВАТЬ ПО
				   ...
				   |	ВзаимозачетЗадолженности.КонтрагентДебитор.ЮрФизЛицо,
				   |	ВзаимозачетЗадолженности.КонтрагентКредитор.ЮрФизЛицо,
				   |	ВзаимозачетЗадолженностиКредиторскаяЗадолженность.НомерСтроки,
				   |	ВЫБОР
				   |		КОГДА ВзаимозачетЗадолженностиКредиторскаяЗадолженность.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
				   |			ТОГДА ЛОЖЬ
				   |		ИНАЧЕ ВЫБОР
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Выборка.Следующий();
		Если Не ЗначениеЗаполнено(Выборка.ОрганизацияКредитор)
			И ((Выборка.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ОплатаПодарочнымСертификатом
				И Выборка.КонтрагентДебитор <> Выборка.КонтрагентКредитор
				И Выборка.КонтрагентДебиторЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
				И Выборка.КонтрагентКредиторЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
				И Выборка.КонтрагентКредитор = Справочники.Контрагенты.РозничныйПокупатель))
			 И Выборка.ТипОбъектаПодарочныйСертификат Тогда
