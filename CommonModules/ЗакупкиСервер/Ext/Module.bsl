...
Процедура ИзвлечьПолеИзКолонкиТаблицы(Таблица,
	...
	Для Каждого КлючЗначение Из НоменклатурныеПозиции Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			Строка.Номенклатура = КлючЗначение.Значение;
		КонецЦикла;
	КонецЦикла;
...
//															если не задано явно.
//		* НоменклатураШапки - СправочникСсылка.Номенклатура, Неопределено - номенклатура комплетка.
...
Функция НайтиИлиСоздатьСоставныеПрослеживаемыеНомераГТД(Товары,
	...
	|	СоставнойНомер.ЕдиницаИзмеренияТНВЭД КАК ЕдиницаИзмеренияТНВЭД,
	// Номенклатура является справочной информацией, и не является ключом при поиске/создании прослеживаемых комплектов.
	|	МАКСИМУМ(СоставнойНомер.Номенклатура) КАК Номенклатура,
	...
	ТекстОшибки = НСтр("ru = 'Для создания составного номера ГТД (РНПТ) требуется подобрать не менее двух номеров ГТД (РНПТ). Подобран один номер: %1.
	...
	НайденныеСтроки = КомплектующиеДляСозданияСоставногоНомераГТД.НайтиСтроки(ОтборСтрок);
	Если НайденныеСтроки.Количество() = 1
		...
		СтрокаГТД = НайденныеСтроки[0];
		ТекстОшибки = СтрШаблон(ТекстОшибки, СокрЛП(СтрокаГТД.НомерРНПТ));
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,,,,Отказ);
	КонецЕсли;
...
//  * КоличествоПоРНПТ - Число -
//  * Количество - Число -  
Функция РазобратьСоставныеПрослеживаемыеНомераГТД(ТоварыКСписанию, ТоварыКОприходованию, Отказ = Ложь) Экспорт
	...
	|	ВтТовары.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.ЕдиницаИзмерения КАК ЕдиницаИзмеренияТНВЭД,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтСписокТоваров.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	ВтСписокТоваров.ВидЗапасов КАК ВидЗапасов,
	|	ВтСписокКомплектующих.НомерГТД КАК НомерГТД,
	|	ВтСписокКомплектующих.ЕдиницаИзмеренияТНВЭД,
	|	ВЫРАЗИТЬ(ВтСписокКомплектующих.Количество / ВтСписокТоваровСумма.ВсегоКоличество * ВтСписокТоваров.Количество КАК ЧИСЛО(23, 11)) КАК
	|		КоличествоПоРНПТ,
	|	ВЫРАЗИТЬ(ВтСписокКомплектующих.Количество / ВтСписокКомплектующихСумма.ВсегоКоличество * ВтСписокТоваров.Количество КАК ЧИСЛО(15, 3)) КАК
	|		Количество
	...
	|		ПО ВтСписокТоваров.ЕдиницаИзмеренияТНВЭД = ВтСписокКомплектующих.ЕдиницаИзмеренияТНВЭД
	...
	КомплектующиеРНПТ = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
	...
	Для Каждого Строка Из КомплектующиеРНПТ Цикл
		...
		Если ИсходноеКоличество <> 0 Тогда
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщения, СокрЛП(Строка.ПрослеживаемыйКомплект), СокрЛП(Строка.НомерГТД), СокрЛП(Строка.ЕдиницаИзмеренияТНВЭД));
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, "", "", Отказ);
		КонецЕсли;
		
		Если Строка.ТребуетсяЗаполнитьНоменклатуруВГТД
			И ЗначениеЗаполнено(Строка.Номенклатура) Тогда
			
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("Справочник.НомераГТД"); 
			ЭлементБлокировки.УстановитьЗначение("Ссылка", Строка.НомерГТД);
			...
			Попытка
				...
				ОбъектГТД = Строка.НомерГТД.ПолучитьОбъект();
				ОбъектГТД.Номенклатура = Строка.Номенклатура;
				ОбъектГТД.НаименованиеТовара = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Номенклатура, "НаименованиеПолное");
				ОбъектГТД.Записать();
...
Функция СоздатьСоставнойПрослеживаемыйНомерГТД(КомплектующиеДляСозданияСоставногоНомераГТД,
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		Если Не Инициализирован Тогда
			...
			Если НоменклатураШапки <> Неопределено Тогда
				...
				Если СоставнойНомерГТДОбъект.СуммаПоРНПТ = 0 Тогда
					СоставнойНомерГТДОбъект.СуммаПоРНПТ = СуммаПоВыбранномуПрослеживаемомуТовару(НоменклатураШапки);
				КонецЕсли;
			КонецЕсли;
		...
		НоваяСтрока.СуммаПоРНПТ = СуммаПоВыбранномуПрослеживаемомуТовару(Строка.Номенклатура, Строка.НомерРНПТ, Строка.КоличествоПоРНПТ);
...
Функция СуммаПоВыбранномуПрослеживаемомуТовару(Номенклатура, НомерРНПТ = Неопределено, КоличествоПоРНПТ = 1)
