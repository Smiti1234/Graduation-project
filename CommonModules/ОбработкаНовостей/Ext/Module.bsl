			...
			Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
				...
				Попытка
					...
					ПараметрыПодключения = Новый Структура(
						"Сервер, Порт, Пользователь, Пароль, Прокси, Таймаут, ЗащищенноеСоединение",
						ИмяСервера_Сервер,
						ИмяСервера_Порт,
						Логин,
						Пароль,
						Прокси,
						НастройкиПолученияНовостей.HTTPСоединениеТаймаутСоединенияСекунд,
						?(ВРег(Протокол) = ВРег("https"), ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(), Неопределено));
