...
// * РазрешительныйРежимДатаЗапросаГИСМТ          - Строка, Неопределено                                                  - Дата получения идентификатора ГИС МТ
	...
	Для Каждого СтрокаТаблицы Из КодыНаОборудованииРозлива Цикл
		...
		Если СтрокаШтрихкодаТекущегоКега = Неопределено Тогда
			Если КоличествоЧастичноеВыбытие > 0 Тогда
				...
				Если СтрокаТаблицы.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
...
Процедура ЗаполнениеСохраняемойСтрокиШтрихкодовУпаковкиПоДаннымРазрешительнойСистемыГИСМТ(СтрокаШтрихкод, ДанныеШтрихкода, ПараметрыСканирования)
	...
	Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПроверкаТоварныхГруппПриРозничнойПродажеГИСМТ() Тогда
		Возврат;
	КонецЕсли;
