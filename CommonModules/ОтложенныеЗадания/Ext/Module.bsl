...
Функция Добавить(Задания, СгенерироватьИдентификаторЗаписи = Ложь) Экспорт
	...
	ПоляЗапуска = ПоляЗапуска();
	
	ОчередиЗаданий = ОчередиЗаданий().Скопировать(, ПоляЗапуска);
	ОчередиЗаданий.Колонки.Добавить("Обработать", Новый ОписаниеТипов("Булево"));
	ОчередиЗаданий.Индексы.Добавить("Очередь");
	ОчередиЗаданий.Индексы.Добавить("Обработать");
	
	ОтборОчередь = Новый Структура("Очередь");
	
	Для каждого Задание Из Задания Цикл
		
		ОтборОчередь.Очередь = Задание.Ключ;
		
		НайденныеСтроки = ОчередиЗаданий.НайтиСтроки(ОтборОчередь);
		НайденныеСтроки[0].Обработать = Истина;
		
	КонецЦикла;
	
	ТаблицаОчередей = ОчередиЗаданий.Скопировать(Новый Структура("Обработать", Истина), ПоляЗапуска);
...
//   * УдалитьЗаданияВТранзакции         - Булево - задания удаляются в транзакции, по умолчанию ЛОЖЬ.
...
Функция ОчередиЗаданий() Экспорт
	...
	ТаблицаОчередей.Колонки.Добавить("УдалитьЗаданияВТранзакции",         ТипБулево);
	...
	Параметры.УдалитьЗаданияВТранзакции   = Истина;
...
Процедура СтартерУправляющегоПотока(Очередь = "") Экспорт
	...
	ТаблицаОчередей = ?(ЗначениеЗаполнено(Очередь),
							ОчередиЗаданий().Скопировать(Новый Структура("Очередь", Очередь)),
							ОчередиЗаданий());
...
Процедура УправляющийПоток(Очередь, ИдентификаторыЗаписей = Неопределено) Экспорт
	...
	УдалитьЗаданияВТранзакции 		  = ПараметрыОчереди.УдалитьЗаданияВТранзакции;
	...
	Иначе
		...
		ОтборПоИдентификаторам           = Истина;
		
		Запрос.УстановитьПараметр("ИдентификаторыЗаписей", ИдентификаторыЗаписей);
		
	КонецЕсли;
	
	Запрос.Текст = ТекстЗапросаВыборкиЗаданий(КоличествоЗаданий, Очередь, АналитикаРасчета, ОтборПоИдентификаторам);
		...
		Если ТекущаяПорцияРасчета = 0 Или КоличествоСтрокКОбработке = 0 Тогда
			...
			Если РежимОтладки Тогда
				...
				ОбщегоНазначения.ВыполнитьМетодКонфигурации(
					ИмяМетода,
					ПараметрыРабочегоПотока(Очередь, ПараметрыДанных(ТаблицаПорции), Ложь, Ложь, Ложь, Истина));
	...
	Если Не ИнформационнаяБазаФайловая И Не РежимОтладки Тогда
...
//  УдалитьЗаданияВТранзакции   - Булево - признак удаления заданий в транзакции.
...
Процедура ВыполнитьЗадания(
	...
	НайденныеСтроки = ОчередиЗаданий().НайтиСтроки(Новый Структура("Очередь", Очередь));
	
	ИмяМетода = НайденныеСтроки[0].ИмяМетода;
...
//  УдалитьЗаданияВТранзакции   - см. ВыполнитьЗадания.УдалитьЗаданияВТранзакции
...
Функция ТекстЗапросаВыборкиЗаданий(КоличествоЗаданий, Очередь, АналитикаРасчета = "", ОтборПоИдентификаторам = Ложь)
...
Процедура ОбработатьОшибки(
			...
			Данные,
			ИдентификаторыНеОбработанныхЗаписей,
			УдалитьЗаданияВТранзакции = Ложь,
			ТекстИсключения = "")
	...
	Если УдалитьЗаданияВТранзакции Тогда
		...
		Попытка
			...
			УдалитьЗадания(Очередь, Данные,, УдалитьЗаданияВТранзакции, ТекстИсключения);
...
Процедура ВыполнитьЗаданияВФонеУправлящегоПотока(
			ИмяМетода,
			Очередь,
			Порция,
			ВыполнитьЗаданияВТранзакции,
...
//  УдалитьЗаданияВТранзакции   - см. ВыполнитьЗадания.УдалитьЗаданияВТранзакции
//
Процедура ДобавитьРабочийПоток(Таблица, Очередь, Данные, ВыполнитьЗаданияВТранзакции, УдалитьЗаданияВТранзакции)
	
	СтрокаТаблицы = Таблица.Добавить();
	СтрокаТаблицы.Данные                    = Данные;
	СтрокаТаблицы.АдресНеОбработанныхДанных = ПоместитьВоВременноеХранилище(Неопределено);
	СтрокаТаблицы.ФоновоеЗадание            = РабочийПоток(
												Очередь,
												СтрокаТаблицы,
												ВыполнитьЗаданияВТранзакции,
												УдалитьЗаданияВТранзакции,
...
Функция РабочийПоток(Очередь, Данные, ВыполнитьЗаданияВТранзакции, УдалитьЗаданияВТранзакции, ЗафиксироватьОшибку)
...
Функция ПараметрыРабочегоПотока(
	...
	ПараметрыМетода.Добавить(УдалитьЗаданияВТранзакции);
...
Функция КонтролироватьРабочиеПотоки(
			РабочиеПотоки,
			Очередь,
			ВыполнитьЗаданияВТранзакции,
			УдалитьЗаданияВТранзакции,
...
Функция ПараметрыОчередиЗаданий()
	...
	Параметры.Вставить("УдалитьЗаданияВТранзакции",         Ложь);
