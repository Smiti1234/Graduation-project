...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		...
		Если ИмяФормы = "Документ.ВозвратВОборотИСМП.Форма.ФормаСпискаДокументов"
			...
			ИЛИ ИмяФормы = "Документ.УточнениеСведенийОКодахМаркировкиИСМП.Форма.ФормаСпискаДокументов" Тогда
		...
		ИначеЕсли ИмяФормы = "Документ.ВозвратВОборотИСМП.Форма.ФормаСписка"
			...
			ИЛИ ИмяФормы = "Документ.УточнениеСведенийОКодахМаркировкиИСМП.Форма.ФормаСписка" Тогда
		...
		ИначеЕсли ИмяФормы = "Документ.ВозвратВОборотИСМП.Форма.ФормаДокумента"
			...
			ИЛИ ИмяФормы = "Документ.УточнениеСведенийОКодахМаркировкиИСМП.Форма.ФормаДокумента" Тогда
...
Процедура ПриИзмененииНоменклатуры(Форма, ТекущаяСтрока, КэшированныеЗначения = Неопределено, ПараметрыУказанияСерий = Неопределено) Экспорт
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "Характеристика", ПроверятьОбъект) Тогда
		ЗаполнитьРеквизит(ДанныеТекущейСтроки, ТекущаяСтрока, "Характеристика", ПроверятьОбъект);
		ЗаполнитьРеквизит(ДанныеТекущейСтроки, ТекущаяСтрока, "ХарактеристикиИспользуются", ПроверятьОбъект);
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "Серия", ПроверятьОбъект) Тогда
		ЗаполнитьРеквизит(ДанныеТекущейСтроки, ТекущаяСтрока, "Серия", ПроверятьОбъект);
		ЗаполнитьРеквизит(ДанныеТекущейСтроки, ТекущаяСтрока, "ТипНоменклатуры", ПроверятьОбъект);
		...
		Если ПараметрыУказанияСерий <> Неопределено Тогда
			...
			ЗаполнитьРеквизит(ДанныеТекущейСтроки, ТекущаяСтрока, "СтатусУказанияСерий", ПроверятьОбъект);
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "Артикул", ПроверятьОбъект) Тогда
		ДанныеТекущейСтроки.Вставить("Артикул");
		СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул", Новый Структура("Номенклатура", "Артикул"));
	КонецЕсли;
	
	Если ЕстьРеквизит(ТекущаяСтрока, "ЕдиницаИзмерения", ПроверятьОбъект) Тогда
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "СтавкаНДС", ПроверятьОбъект) Тогда
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "СуммаНДС", ПроверятьОбъект)
		И ЕстьРеквизит(ТекущаяСтрока, "СтавкаНДС", ПроверятьОбъект) Тогда
		ДанныеТекущейСтроки.Вставить("СуммаНДС", 0);
		ДанныеТекущейСтроки.Вставить("СтавкаНДС");
		СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	КонецЕсли;
	
	Если ЕстьРеквизит(ТекущаяСтрока, "СуммаСНДС", ПроверятьОбъект) Тогда
		ДанныеТекущейСтроки.Вставить("СуммаСНДС", 0);
		СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	КонецЕсли;
	
	Если ЕстьРеквизит(ТекущаяСтрока, "Сумма", ПроверятьОбъект) Тогда
		ДанныеТекущейСтроки.Вставить("Сумма", 0);
		СтруктураДействий.Вставить("ПересчитатьСумму");
	КонецЕсли;
	
	Если ЕстьРеквизит(ТекущаяСтрока, "Упаковка", ПроверятьОбъект) Тогда
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "КодТНВЭД", ПроверятьОбъект) Тогда
	...
	Если ЕстьРеквизит(ТекущаяСтрока, "СкоропортящаясяПродукция", ПроверятьОбъект) Тогда
...
Функция ЕстьРеквизит(ТекущаяСтрока, Имя, ПроверятьОбъект)
...
Процедура ЗаполнитьРеквизит(Приемник, Источник, Имя, ПроверятьОбъект)
