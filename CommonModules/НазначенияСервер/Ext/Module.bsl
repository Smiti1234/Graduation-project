...
Процедура ПроверитьЗаполнитьНазначенияСтрокахКоллекции(Массив, НазначениеДляСопоставления, НазначениеПоУмолчанию)
	...
	Запрос.Текст =
		...
		|		И ВЫБОР
		|			КОГДА СпрНазначения.Ссылка ЕСТЬ NULL ТОГДА
		|				&НазначениеПоУмолчанию <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|			КОГДА НазначениеДляСопоставления.Ссылка ЕСТЬ NULL ТОГДА
		|				НЕ СпрНазначения.ТипНазначения В(
