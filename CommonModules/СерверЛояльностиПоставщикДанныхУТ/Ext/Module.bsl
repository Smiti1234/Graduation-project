...
Процедура СоздатьКартуКлиента(ДанныеКарты, Результат = Неопределено, ДругойКлиент = Неопределено) Экспорт
	...
	Если Не ПроверитьДанныеКарты(ДанныеКарты.ИдентификаторКартыКлиента, ВидКартыЛояльности) Тогда
		Описание = НСтр("ru='Номер карты введен неверно'");
		ЗаписьЖурналаРегистрации(НСтр("ru = 'СерверЛояльности.СоздатьКартуКлиента'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ДесериализоватьПараметрыРасчета(ВходящиеДанные, Результат) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'СерверЛояльности.АвтоматическийРасчетСкидок'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			Описание);
...
Функция ПолучитьТаблицуОстатков(ПараметрыПолученияОстатков) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ОстаткиТоваров.Склад) КАК МестоХранения,
...
Процедура ПараметрыИзЗапросаПолучитьОстатки(ВходящиеДанные, Результат) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'СерверЛояльности.ПолучитьОстатки'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			Описание);
