...
Функция ВариантОбеспеченияПриИзменении(Форма, ТекущаяСтрока) Экспорт
	...
	Если ВариантОбеспеченияВыбранноеЗначение.ВариантОбеспечения
				= ПараметрыВстраивания.СлужебныеПараметры.ВариантОбеспеченияДоИзменения
			И Не ВариантОбеспеченияВыбранноеЗначение.РазбитьСтроку Тогда
...
Функция ПараметрыВыбораОбеспечения(Форма, ТекущаяСтрока, Режим, Склады = Неопределено) Экспорт
	...
	СкладыГруппы = ?(Склады = Неопределено, СкладыГруппы(Запрос, Истина), Склады);
...
Функция ТекстЗапросаДоступныхОстатковПоДатамДляПодбора(ЕстьХарактеристика, СоставРазделовЗапроса) Экспорт
	...
	ТекстЗапроса =
		...
		|	Набор.Период
		|ИМЕЮЩИЕ
		|	СУММА(Набор.ВНаличии) <> 0 ИЛИ СУММА(Набор.Свободно) <> 0
		|ИНДЕКСИРОВАТЬ ПО
		|	Склад
		|;
		|
...
Функция ВременнаяТаблицаСвободныеОстатки(ИмяТаблицыФильтра, ИмяТаблицыРезультат, ПолучатьОтрицательные = Ложь) Экспорт
...
Процедура ОбменыПослеЗагрузкиДанных(НоменклатураСоответствие) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Запрос.Выполнить();
	Документ = ПроведениеДокументов.ЭмуляцияДокумента(Документы.КорректировкаРегистров.ПустаяСсылка(), Неопределено); // нужна ссылка на заказ, для механизма состояний заказов клиентов
	РаспределениеЗапасовДвижения.ОтразитьЗаданияКРаспределениюЗапасов(Новый Структура("Ссылка", Неопределено), МенеджерВременныхТаблиц);
...
Процедура ЗаполнитьТаблицыИзДанныхФормы(Таблицы, Форма)
...
Функция НуженВопросОбОтгрузкеОднойДатой(ПараметрыВстраивания, Заказы)
	...
	Текст =
		...
		|ГДЕ
		|	Заказы.РезервироватьПоМереПоступленияОстаток > 0
		|		ИЛИ Заказы.КОбеспечениюОстаток > 0
