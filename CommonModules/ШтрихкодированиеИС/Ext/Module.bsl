...
Функция ВложенныеШтрихкодыУпаковок(Упаковки, ПараметрыСканирования = Неопределено, ЗаполнитьСведенияПоШтрихкодамEAN = Ложь, СортироватьДанные = Ложь) Экспорт
	...
	Если ТипЗнч(Упаковки[0]) <> Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров") Тогда
		...
		Для Каждого ЭлементМассиваУпаковок Из Упаковки Цикл
			...
			Если ЗначениеЗаполнено(ЭлементДанныхГИСМТ.РазрешительныйРежимИдентификаторЗапросаГИСМТ) Тогда
...
Функция ИнициализацияТаблицыДанныхКодовМаркировки(ИндексироватьТаблицы = Истина) Экспорт
	...
	// флаг устанавливается, если наличие идентификатора запроса или признаки КМ можно игнорировать
	ДанныеКодовМаркировки.Колонки.Добавить("РазрешенаПродажа",                             Новый ОписаниеТипов("Булево"));
		...
		Если ОписаниеНоменклатуры <> Неопределено Тогда
			...
			Если ОписаниеНоменклатуры.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
				Если НастроеннаяНоменклатураЧастичногоВыбыти = Неопределено Тогда
					НастроеннаяНоменклатураЧастичногоВыбыти = РегистрыСведений.НастройкиЧастичногоВыбытияТоваровИСМП.НастроеннаяНоменклатураЧастичногоВыбытия(НаборНоменклатуры);
				КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			Если Не КодМаркировкиЗаполненПоПулу И ТребуетсяСопоставление Тогда
				...
				Если ТребуетсяЧастичноеВыбытие(ПараметрыСканирования, ДанныеПоШтрихкодам)
					И СтрокаДанных.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
		...
		Если ПодменныеНоменклатурыЧастичногоВыбытие.Количество() Тогда
			...
			Если СтрокиПодменнойНоменклатуры.Количество() > 0 Тогда
				...
				СтрокаДанных.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура;
			КонецЕсли;
