...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Реализация без перехода права собственности для комиссионного товара должна делать движения по разделу ТоварыПринятыеНаКомиссиюВПути
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|	ИЗ
	|		ВТДвиженияСебестоимостьТоваров КАК Т
	|	    ЛЕВОЕ СОЕДИНЕНИЕ РеализацииВПутиРазныйРазделУчета КАК Реализации
	|		    ПО Реализации.Регистратор = Т.Регистратор
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.РеализацияТоваровУслуг)
	|		И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	...
	|		И Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|       И Реализации.Регистратор ЕСТЬ NULL
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
