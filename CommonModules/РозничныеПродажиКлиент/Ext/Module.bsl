...
Процедура ОткрытьКассовуюСменуЗавершение(Результат, Параметры) Экспорт
	...
	Иначе
		...
		ПараметрыОперации.Кассир = Параметры.РеквизитыКассира.Наименование;
		ПараметрыОперации.КассирИНН = Параметры.РеквизитыКассира.ИНН;
...
Процедура ПродолжитьПодключениеОборудованиеПоИдентификаторуПоследовательно(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Иначе
			...
			РозничныеПродажиВызовСервера.ЗаписьЖурналаУровеньОшибка(НСтр("ru = 'Розничные продажи'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),,,ТекстСообщения);
