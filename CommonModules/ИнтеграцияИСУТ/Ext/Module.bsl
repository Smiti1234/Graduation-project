...
//
// Параметры:
//  ТабличнаяЧасть  - ТабличнаяЧасть - табличная часть документа ГосИС в которую надо указать связанные серии, с колонками:
...
//  ДокументОснование - ДокументСсылка - прикладной документ по которому заполняется документ ГосИС
//  РеквизитыСерий - Структура - реквизиты табличной части заполняемые из серии
//                 - Неопределено - если нужно заполнить только серию
//
Процедура ЗаполнитьСерииПоСкладскимДвижениям(ТабличнаяЧасть, ДокументОснование, РеквизитыСерий = Неопределено) Экспорт
		...
		Если ЕстьИСМП Тогда
			Для Каждого ВидПродукцииИС Из ИнтеграцияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции() Цикл
				...
				Если ВидПродукцииИС = Перечисления.ВидыПродукцииИС.Пиво Тогда
					Реквизит = Новый РеквизитФормы("НоменклатураПодключенаКОборудованиюРозливаИСМП", Новый ОписаниеТипов("Булево"), "Объект.Товары");
					ДобавляемыеРеквизиты.Добавить(Реквизит);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
// Заполняет коды маркировки кег на оборудовании розлива ИСМП
// 
// Параметры:
...
Процедура ЗаполнитьКодыМаркировкиНаОборудованииРозливаИСМП(Форма, СтрокиТоваров, ПрименятьКешШтрихкодов = Ложь) Экспорт
	...
	Если Результат.ЕстьИзменения Тогда
		...
		Для Каждого КлючИЗначение Из Результат.ДобавленныеКодыМаркировки Цикл
			...
			НоваяСтрока.АкцизнаяМарка = КлючИЗначение.Значение.ШтрихкодУпаковки;
		...
		Для Каждого КлючИЗначение Из Результат.ИзмененныеКодыМаркировки Цикл
			...
			СтруктураПоиска.АкцизнаяМарка = КлючИЗначение.Значение.ШтрихкодУпаковки;
			...
			Если ПоискСтрок.Количество() Тогда
				КоличествоДоИзменения    = ПоискСтрок[0].ЧастичноеВыбытиеКоличество;
				ЗаполнитьЗначенияСвойств(ПоискСтрок[0], КлючИЗначение.Значение);
				КоличествоПослеИзменения = ПоискСтрок[0].ЧастичноеВыбытиеКоличество;
			КонецЕсли;
			Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧСерии(СтрокаТовары.СтатусУказанияСерий, ПараметрыУказанияСерий)
				...
				Если СтрокиСерий.Количество() Тогда
					СтрокиСерий[0].Количество = СтрокиСерий[0].Количество + (КоличествоПослеИзменения - КоличествоДоИзменения);
