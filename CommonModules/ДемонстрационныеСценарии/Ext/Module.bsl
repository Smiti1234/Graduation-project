...
Функция СтрокаПредставлениеСЕще(ЗаголовокПредставления, СписокПредставлений, МаксимальнаяДлинаСтроки) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокПредставлений Цикл
		...
		Иначе
			Возврат Представление + " " + СтрШаблон(НСтр("ru = 'и еще %1'; en = 'and more %1'"), Строка(СписокПредставлений.Количество() - ОбработаноЭлементов));
		КонецЕсли; 
...
//
// Возвращаемое значение:
//   Массив - доступные варианты отображения
//
...
Функция РезультатСозданияПрисоединенногоФайлаСценария(ВладелецФайла, Форма, КодЯзыка) Экспорт
	...
	ПараметрыЗаписиФайла.ИмяБезРасширения            = СтрШаблон("%1_%2_%3", "ОписаниеСценария_" ,
	                                                             ДемонстрационныеСценарииКлиентСервер.ПолныйКодСтрокойДляИмениФайла(СправочникОбъект.ПолныйКод), 
	                                                             КодЯзыка);
...
//  НовыйТекст       - Строка  - новое содержимое файла
//
// Возвращаемое значение:
//   Булево - Истина, если успешно
//
Функция РезультатОбновленияПрисоединенногоФайла(ПрисоединенныйФайл, УникальныйИдентификатор, НовыйТекст) Экспорт
	...
	ДвоичныеТекстаСценария = ПолучитьДвоичныеДанныеИзСтроки(НовыйТекст);
	АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеТекстаСценария, УникальныйИдентификатор);
...
Функция ТекстHTMLПоДаннымПрисоединенногоФайла(ДанныеФайла)
