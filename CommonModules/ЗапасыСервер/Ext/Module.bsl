...
Процедура ТаблицаОстатковПереданнойВозвратнойТары(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	ТаблицаДанныхДокумента.Склад КАК СкладОтгрузки,
	...
	|	ПереданнаяВозвратнаяТараОстатки.КорВидЗапасов КАК КорВидЗапасов,
	|	ПереданнаяВозвратнаяТараОстатки.НомерГТД КАК НомерГТД,
	|	ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка) КАК Сделка,
	|	НЕОПРЕДЕЛЕНО КАК ВидЗапасовПолучателя,
	|	НЕОПРЕДЕЛЕНО КАК ДатаПоступления,
	|	СУММА(ПереданнаяВозвратнаяТараОстатки.КоличествоОстаток) КАК КоличествоОстаток,
	|	СУММА(ПереданнаяВозвратнаяТараОстатки.КоличествоПоРНПТОстаток) КАК КоличествоПоРНПТОстаток,
	|	0 КАК СуммаОстаток
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ
	...
	|	ПереданнаяВозвратнаяТараОстатки.КорВидЗапасов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаУчетаНоменклатуры
	|;
...
Функция ОписаниеТаблицСОбязательнымиКолонками()
	...
	ОписаниеТаблиц.ИсходнаяТаблицаТоваров.Вставить("Валюта", "ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)");
