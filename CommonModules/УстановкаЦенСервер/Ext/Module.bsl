				...
				Если ТекущаяЦена = 0 Или Не ТолькоНезаполненные Тогда
					...
					Если ЗначениеЗаполнено(Формула) Тогда
						...
						Попытка
							...
							СтрокаТаблицыЦен["ФормулаИзмененаВручную" + ВидЦены.ИмяКолонки] = (ПредставлениеФормулы <> ПредставлениеФормулыПоУмолчанию);
...
Процедура ЗагрузитьТабличнуюЧастьТовары(Форма, КэшДанных) Экспорт
	...
	Иначе
		...
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		РегистрСведений.ЦеныНоменклатурыПоставщиков.СрезПоследних(&Дата, (Номенклатура, Характеристика,
		|			ВидЦеныПоставщика) В
		|			(Выбрать
		|				Т.Номенклатура,
		|				Т.Характеристика,
