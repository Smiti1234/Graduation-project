...
Функция СписокКонтрагентов(ПараметрыПоиска, НомерСтраницы, КоличествоЭлементовНаСтранице) Экспорт
	...
	Если ПараметрыПоиска.Свойство("ВидАдреса") Тогда

		Если ПараметрыПоиска.ВидАдреса = Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента Тогда
			ВидАдреса = "legalAddress";
		Иначе
			ВидАдреса = "factualAddress";
		КонецЕсли;
