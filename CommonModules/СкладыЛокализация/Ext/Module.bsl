...
Функция ИспользуетсяМаркируемаяПродукция() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		Результат = ИнтеграцияИСМП.ИспользуетсяМаркируемаяПродукция();
	КонецЕсли;
	//-- Локализация
	Возврат Результат;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииГосИС") Тогда
		...
		Если Не (ПараметрыИнтеграции = Неопределено) Тогда
			Для Каждого ВидПродукции Из ПараметрыИнтеграции.ВидыПродукции Цикл
				...
				ТабличнаяЧастьТовары = Форма.Объект[Настройки.ИмяТабличнойЧастиТовары];
				
				ЕстьМаркируемаяПродукция = Ложь;
				
				ПроверкаИПодборПродукцииИСМПУТ.ЕстьМаркируемаяПродукцияВКоллекции(
					ТабличнаяЧастьТовары, ВидПродукции, ЕстьМаркируемаяПродукция);
