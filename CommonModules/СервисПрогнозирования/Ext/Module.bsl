...
Функция СоздатьКонфигурациюМодели(ВидПлана) Экспорт
	...
	Для Каждого КлючЗначение Из НастройкиСервиса.Коллекции Цикл
		Если КлючЗначение.Значение.Выгружать Тогда
			...
			Если ЗначениеЗаполнено(ИнфоКоллекции.ИдКоллекции)
				И Число(ИнфоКоллекции.ИдКоллекции) > 0 Тогда
				ТелоЗапросаКоллекции.Вставить(ИмяВСервисе, Число(ИнфоКоллекции.ИдКоллекции));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если НастройкиВидаПлана.ЗаполнятьСкладВТЧ Тогда
		РазрезыПрогнозирования.Добавить("shop");
	КонецЕсли;
	Если НастройкиВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПродаж Тогда
		Если НастройкиВидаПлана.ЗаполнятьПартнераВТЧ Тогда
			РазрезыПрогнозирования.Добавить("customer");
		КонецЕсли;
		Если НастройкиВидаПлана.ЗаполнятьПоХарактеристикамНоменклатуры Тогда
			РазрезыПрогнозирования.Добавить("item_variant");
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям Тогда
		ТелоЗапроса.Вставить("category_processing", ?(НастройкиВидаПлана.ВариантПрогнозированияПоКатегориям = 0, "group", "reduce"));
	Иначе
		ТелоЗапроса.Вставить("category_processing", "no");
	КонецЕсли;
	ТелоЗапроса.Вставить("target_column", "quantity");
...
Функция ПолучитьПрогнозСервиса(ВидПлана, ИдОбучения, ИмяМодели = Неопределено) Экспорт
	...
	ПараметрыАдреса.Добавить("models");
	ПараметрыАдреса.Добавить(ИдМодели);
	ПараметрыАдреса.Добавить("process");
	
	ПараметрыЗапроса = Новый Соответствие();
	Если ЗначениеЗаполнено(ИмяМодели)
		И ИмяМодели <> "auto" Тогда
		ПараметрыЗапроса.Вставить("model_name",  ИмяМодели);
		ПараметрыЗапроса.Вставить("target_data", "predictions_full");
		ПрогнозПоОптимальнойМодели = Ложь;
	Иначе
		ПараметрыЗапроса.Вставить("target_data", "predictions");
		ПрогнозПоОптимальнойМодели = Истина;
	КонецЕсли;
	...
	Если НастройкиСервиса.РазрешенОбменССервисом Тогда
		...
		ДополнительныйПараметр.Вставить("ПрогнозПоОптимальнойМодели", ПрогнозПоОптимальнойМодели);
...
Функция ВыгрузитьКатегориальнуюКоллекцию(ИмяКоллекции)
	...
	Для Каждого Диапазон Из ДиапазоныВыгрузки Цикл
		...
		ПреобразованныеДанные = ТаблицуЗначенийВМассивСтруктур(ВыгружаемыеДанные, ДоступныеРеквизиты); // Диапазон
...
Функция ВыгрузитьПериодическуюКоллекцию(ИмяКоллекции)
	...
	Пока ЕстьНевыгруженныйПериод(ИмяКоллекции) Цикл
		...
		Если ПорцияВыгрузки.Свойство("ПричинаОшибки") Тогда
			ЕстьОшибки = Истина;
			Если ПорцияВыгрузки.ПричинаОшибки = "НеУказанКалендарьПредприятия" Тогда
				ТекстОшибки = ТекстОшибкиНеУказанКалендарьПредприятия();
			ИначеЕсли ПорцияВыгрузки.ПричинаОшибки = "НеУказанСценарийПрогнозированияКонтрольныхПланов" Тогда
		...
		Если ПорцияВыгрузки.КоличествоДанных > 0 Тогда
			...
			ПреобразованныеДанные = ТаблицуЗначенийВМассивСтруктур(ПорцияВыгрузки.Коллекция, ДоступныеРеквизиты);
			ТелоЗапросаJSON = СформироватьТелоЗапросаJSON(ПреобразованныеДанные, ЕстьОшибки);
...
Функция ТаблицуЗначенийВМассивСтруктур(ТаблицаЗначений, ДоступныеРеквизиты = Неопределено,
	...
	ИначеЕсли ИмяДействия = "ПолучениеПрогнозаСервиса" Тогда
		...
		Если ПланПродажПоКатегориям И НастройкиВидаПлана.ВариантПрогнозированияПоКатегориям = 0 Тогда
			СервисПрогнозированияПереопределяемый.ЗаписатьПрогнозПродажПоКатегориям(СтруктураОтвета,
				ДополнительныйПараметр.ВидПлана,
				ДополнительныйПараметр.ПрогнозПоОптимальнойМодели,
				ДополнительныйПараметр.ИдОбучения,
...
Функция ШаблонНастроекСервиса()
	...
	ВсеНастройкиСервиса.Вставить("Версия", 1.1);
...
Функция СоответствиеИменСвойств(ВключаяОбратные = Истина)
	...
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Категории, "product_categories");
	...
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Заказы, "customer_orders");
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Промо, "promo");
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.ПланыПродаж, "sales_plans");
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Сезонность, "seasonality");
	СоответствиеИмен.Вставить(Перечисления.КоллекцииСервисаПрогнозированияПродаж.Календарь, "calendar");
...
Функция ТекстОшибкиНеУказанКалендарьПредприятия()
	
	Возврат НСтр("ru = 'В информационной базе не указан основной график работы предприятия.
		|Для возможности учета календаря предприятия при прогнозировании, установите график работы предприятия в разделе НСИ и администрирование -
		|Настройка НСИ и разделов - Предприятие - Организации.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "СервисПрогнозирования.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.14.18";
	...
	Изменяемые.Добавить(Метаданные.Константы.НастройкиСервисаПрогнозирования.ПолноеИмя());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	ШаблонНовыхНастроек = ШаблонНастроекСервиса();
	
	Если НастройкиПрогнозирования = Неопределено Тогда
		ОбновитьНастройкиСервиса(ШаблонНовыхНастроек);
		Возврат;
	КонецЕсли;
	
	НастройкиПрогнозирования.Версия = ШаблонНовыхНастроек.Версия;
	
	Ключи           = КлючиКОбработке();
	СтарыеКоллекции = НастройкиПрогнозирования.Коллекции;
		...
		Для Каждого НовоеСвойство Из НоваяКоллекция Цикл
			...
			Если ТипЗнч(СтароеЗначениеСвойства) = Тип("Структура") Тогда
				...
				Для Каждого НовыйРеквизит Из НовоеСвойство.Значение Цикл
					...
					// Исключение актуального реквизита, ошибочно перезаписанного в состав пользовательских реквизитов.
	...
	НастройкиПрогнозирования.Коллекции = НовыеКоллекции;
	ОбновитьНастройкиСервиса(НастройкиПрогнозирования);
...
Функция СоответствиеКоллекцийИсточника(Коллекция)
	...
	ИначеЕсли Коллекция = "Категории" Тогда
	...
	ИначеЕсли Коллекция = "Заказы" Тогда
	...
	ИначеЕсли Коллекция = "ПланыПродаж" Тогда
	...
	ИначеЕсли Коллекция = "Календарь" Тогда
