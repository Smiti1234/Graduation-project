...
Процедура ЗарегистрироватьПроблемуВыполненияРасчета(ПараметрыРегистрации, ТекстКратко,
	...
	ПродублироватьВЖурналРегистрации(ПараметрыРегистрации.ЭтапРасчета, ВажностьПроблемы, ТекстПодробно);
...
//
Процедура ЗафиксироватьРасчет(СледующийМесяц, ОперацияРасчета, ДанныеКРаспределению) Экспорт
...
Процедура УдалитьЗаданияКРасчетуЗаТекущийПериод(ПараметрыОбработчика, ДанныеКРасчету = Неопределено) Экспорт
	...
	Попытка
		...
		ЗафиксироватьРасчет(Неопределено, ПараметрыОбработчика.ДанныеЭтапа.Код, ДанныеКРасчету);
...
Функция ТекстОшибкиЗаписиЗаданияВЗаблокированномПериоде(Организация, ПериодЗадания, ПериодБлокировки, ИмяРегистраЗаданий = Неопределено, ИзменяемыйДокумент = Неопределено)
