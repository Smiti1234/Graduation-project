...
Функция ПолучитьУсловияПродажПоУмолчанию(
	...
	Если Не ЗначениеЗаполнено(СтруктураРеквизитов.Валюта) Тогда
		СтруктураРеквизитов.Валюта = ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СтруктураРеквизитов.ВалютаВзаиморасчетов) Тогда
		СтруктураРеквизитов.ВалютаВзаиморасчетов = ЗначениеНастроекПовтИсп.ВалютаУправленческогоУчета();
	КонецЕсли;
	...
	Иначе
		...
		Для каждого ДокументыПоТипу Из ВыгрузкаДокументовПоТипу.Строки Цикл
			...
			Если ДокументыПоТипуСтроки.Количество() = 0 Тогда
				...
				ТекстОформить = СтрШаблон(ТекстОформить, НРег(НаименованияДокумента.ВинительныйПадеж));
...
Функция ТекстЗапросаПоФактическойОтгрузке(СтруктураДокументаДляЗаполнения, СтруктураРезультатаЗапроса = Неопределено)
	...
	|	ВЫБОР
	|		КОГДА ИСТИНА
	|			ТОГДА -СУММА(ЗаказыОстатки.КОформлениюРасход)
	...
	|		ВЫБОР
	|			КОГДА ИСТИНА
	|				ТОГДА ЗаказыОстатки.СуммаКонечныйОстаток
	|		КОНЕЦ КАК Сумма,
	|		ВЫБОР
	|			КОГДА ИСТИНА
	|				ТОГДА ЗаказыОстатки.КОформлениюКонечныйОстаток
	...
	|		ВЫБОР
	|			КОГДА ИСТИНА
	|				ТОГДА ЗаказыОбороты.СуммаРасход
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ИСТИНА
	|				ТОГДА ЗаказыОбороты.КОформлениюРасход
	|		КОНЕЦ
	|	ИЗ
...
Процедура ВыполнитьКонтрольЗаказаПослеПроведения(КонтролируемыйЗаказ, Отказ) Экспорт
	...
	Запрос = Новый Запрос("
		|ВЫБРАТЬ
		|	ЗаказыКлиентовДанные.ЗаказКлиента КАК ЗаказКлиента
		|ИЗ
		|	РегистрНакопления.ЗаказыКлиентов.Остатки(, 
		|			ЗаказКлиента = &Ссылка) КАК ЗаказыКлиентовДанные
		|ГДЕ
		|	&КонтролироватьОтгрузку
		|	И (ЗаказыКлиентовДанные.ЗаказаноОстаток <> 0 
		|		ИЛИ ЗаказыКлиентовДанные.КОформлениюОстаток <> 0)
		...
		|		Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
		|	ГДЕ
		|		ОбъектыРасчетов.Объект = &Ссылка) КАК ЗаказыКлиентовДанные
		|ГДЕ
	...
	Если ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		УсловиеЗаказКлиентаСтатус = "
		|   ВЫРАЗИТЬ(ЗаказыКлиентовДанные.ЗаказКлиента КАК Документ.ЗаказКлиента).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.Закрыт)"; 

		УсловиеЗаказКлиентаХозяйственнаяОперация = "
		|   ВЫРАЗИТЬ(ЗаказыКлиентовДанные.ЗаказКлиента КАК Документ.ЗаказКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
		| И ВЫРАЗИТЬ(ЗаказыКлиентовДанные.ЗаказКлиента КАК Документ.ЗаказКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)";
	...
	ИначеЕсли ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		УсловиеЗаказКлиентаСтатус = "
		|   ВЫРАЗИТЬ(ЗаказыКлиентовДанные.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена)";

		УсловиеЗаказКлиентаХозяйственнаяОперация = "
		|   ВЫРАЗИТЬ(ЗаказыКлиентовДанные.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера)";
