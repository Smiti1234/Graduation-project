...
Функция РаспределитьШтрихкодыПоТаблицеТоваров(ДокументСсылка, МассивУпаковок, ПараметрыУказанияСерий, Товары) Экспорт
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Для Каждого СтрокаРаспределения Из РаспределениеПоСтроке Цикл
			...
			Если СтрокаТовары.КоличествоУпаковок <> СтрокаТовары.Количество
				И СтрокаРаспределения.ВидУпаковки <> Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
			...
			ИначеЕсли СтрокаРаспределения.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				...
				Если СтрокаТовары.КоличествоУпаковок <> СтрокаТовары.Количество И ЗначениеЗаполнено(СтрокаТовары.Упаковка) Тогда
					РеквизитыУпаковки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТовары.Упаковка, "Знаменатель,Числитель");
					КоэффициентПересчетаУпаковки = РеквизитыУпаковки.Знаменатель / РеквизитыУпаковки.Числитель;
				Иначе
...
Процедура УстановитьВозможностьИспользованиеОплатыСБПНаФормахРМК(Форма) Экспорт
	...
	Если Справочники.НастройкиРМК.СписокДоговоровИПодключенийСБП(Форма.Объект.Организация).Количество() Тогда
...
//  	*ДокументОплаты - см. ИнтеграцияСПлатежнымиСистемами.АктивироватьКассовуюСсылку.ДокументОплаты 
//  	*ДанныеСсылки - см. ИнтеграцияСПлатежнымиСистемами.АктивироватьКассовуюСсылку.ДанныеСсылки 
//  	*ТорговаяТочка - см. ИнтеграцияСПлатежнымиСистемами.АктивироватьКассовуюСсылку.ДокументОплаты 
//  АдресРезультата - Строка - Адрес результата
...
//  ПараметрыПроцедуры - Структура:
//  	*ДокументОплаты - см. ИнтеграцияСПлатежнымиСистемами.АктивироватьКассовуюСсылку.ДокументОплаты 
//  	*ДанныеСсылки - см. ИнтеграцияСПлатежнымиСистемами.ДеактивироватьКассовуюСсылку.ДанныеСсылки 
//  	*ТорговаяТочка - см. ИнтеграцияСПлатежнымиСистемами.ДеактивироватьКассовуюСсылку.ТорговаяТочка 
//  АдресРезультата - Строка - Адрес результата
Процедура ДеактивироватьКассовуюСсылку(ПараметрыПроцедуры, АдресРезультата) Экспорт
	
	ПередВыполнениемОперацииВПлатежнойСистеме(ПараметрыПроцедуры.ДокументОплаты);
	
	РезультатОперации = ИнтеграцияСПлатежнымиСистемами.ДеактивироватьКассовуюСсылку(
		ПараметрыПроцедуры.ДанныеСсылки,
		ПараметрыПроцедуры.ТорговаяТочка);
