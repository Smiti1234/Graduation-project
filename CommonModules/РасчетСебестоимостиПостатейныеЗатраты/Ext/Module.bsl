...
Функция ТекстПриемникиПоТоварамПрошлыхПериодовОтклоненийВСтоимости()
	...
	ТекстЗапроса = "
		...
		|ГДЕ
		|	(&РаспределениеДопРасходовПоВыбывшимТоварам
		// Доп расходы по товарам в пути от поставщиков всегда распределяем на выбытия товаров в прошлых периодах.
		|		ИЛИ Источник.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути))
...
Функция ТекстПриемникиПоТоварамПрошлыхПериодовДополнительныхРасходов()
	...
	ТекстЗапроса =
		...
		|		И НЕ Реестр.ДополнительнаяЗапись
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрАналитики
		|		ПО Реестр.Ссылка = Реквизиты.АналитикаРасходов
		|		И НЕ Реестр.ДополнительнаяЗапись
		|ГДЕ
		|	Источник.ЭтоПартияПрошлогоПериода
...
Функция ТекстЗапросаВыборкаРаспределениеРасходовНаЗаказКлиента() 
	...
	|				И Расходы.Организация = База.Организация
	|				И Расходы.НаправлениеДеятельности = База.НаправлениеДеятельностиКонтрагента)
	|;
	|
	|";
