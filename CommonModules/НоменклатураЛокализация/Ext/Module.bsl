...
//	ДатаОкончанияСрокаДействия - Строка - дата окончания срока действия сертификата номенклатуры.
...
//
Функция ЗапроситьИнформациюУРосаккредитации(Номер, ДатаНачалаСрокаДействия, ДатаОкончанияСрокаДействия, ТипСертификата) Экспорт
	
	Ответ = Новый Структура("ТекстОшибки", "");
	//++ Локализация
	Если РаботаСНоменклатуройСлужебный.СервисАккредитацииДоступен() Тогда
		ПараметрыЗапроса = РаботаСНоменклатуройСлужебный.НовыеПараметрыДокументаАккредитации();
		ПараметрыЗапроса.Номер = Номер;
...
Процедура УстановитьТекстЗапросаОтчетаТоварыСИстекающимиСертификатами(СхемаКомпоновкиДанных) Экспорт
	...
	|	ВЫРАЗИТЬ(ТипыСертификатовПоОбластямДействия.ТипСертификата КАК СТРОКА(25)) КАК ТипСертификата,
	|	ВЫБОР
