...
Процедура ПроверитьЗаполнениеОплатыПлатежнымиКартами(Объект, Отказ) Экспорт
	...
	Для Каждого СтрокаТЧ Из Объект.ОплатаПлатежнымиКартами Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ЭквайринговыйТерминал)
			...
			Текст = НСтр("ru='В строке %НомерСтроки% табличной части """"Оплата платежными картами"""" необходимо указать эквайринговый терминал.'");
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМ") 
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.ДоговорПодключения)
				...
				Текст = НСтр("ru='В строке %НомерСтроки% табличной части """"Оплата платежными картами"""" необходимо указать договор подключения.'");
				Текст = СтрЗаменить(Текст, "%НомерСтроки%",   СтрокаТЧ.НомерСтроки);
...
Процедура ЗаполнитьПараметрыОтправкиЭлектронногоЧекаПоПартнеру(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.Партнер) И Форма.Партнер <> Справочники.Партнеры.РозничныйПокупатель Тогда
		ПартнерЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Форма.Партнер, "ВариантОтправкиЭлектронногоЧека,ЮрФизЛицо");
		Если ПартнерЗначенияРеквизитов.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо Тогда
			
			ВариантОтправкиЭлектронногоЧека = ПартнерЗначенияРеквизитов.ВариантОтправкиЭлектронногоЧека;
