...
Процедура РаспределениеПартийНДСФИФОСкользящая(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьПодготовкуДанныхДляСледующихЭтапов(
		ПараметрыРасчета,
		"ПриходыТоваровНДС2_4, ВТКоэффициентыДопРасходов, ПриходыТоваровНДС2_4_ДляРешенияСЛУ");
	...
	НовыеВТ = РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьПереченьСуществующихВременныхТаблиц(
		ПараметрыРасчета,
		СуществующиеВТ + ", ПриходыТоваровНДС2_4, ВТКоэффициентыДопРасходов");
...
Процедура СформироватьСтоимостьПартийНДСНаКонецПериода(ПараметрыРасчета, НомерВызова)
	...
	|
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.СтоимостьСНДС КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК СтоимостьСНДС,
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.СтоимостьБезНДС КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК СтоимостьБезНДС,
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.СтоимостьБезНДСРегл КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК СтоимостьБезНДСРегл,
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.НДСРегл КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК НДСРегл,
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.СтоимостьБезНДСУпр КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК СтоимостьБезНДСУпр,
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(Цены.НДСУпр КАК ЧИСЛО(31,10)) * ВЫРАЗИТЬ(Т.Количество КАК ЧИСЛО(15,3)) КАК ЧИСЛО(31,10)) КАК НДСУпр,
	|	Т.Количество
...
Функция ТекстЗапросаПодготовкаДанныхРаспределенияНДС(ПараметрыРасчета = Неопределено) Экспорт
	...
	|		И Т.АналитикаФинансовогоУчета = Остатки.АналитикаФинансовогоУчета
	|ГДЕ
	|	Остатки.ЭтоПостатейныеПеременные ИЛИ Остатки.ЭтоПостатейныеПостоянные
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Партия
	...
	|		КОГДА НЕ ДокументыСторно.Сторно ЕСТЬ NULL ТОГДА ИСТИНА
	...
	|	&НачалоПериода,
	|	НЕОПРЕДЕЛЕНО,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Прошлое),
	|	НЕОПРЕДЕЛЕНО,
	|	ЛОЖЬ КАК Сторно,
...
Процедура ВключитьИсключитьНДСВСтоимость24(ПараметрыРасчета) Экспорт
	...
	|	УчетСебестоимости.КорАналитикаФинансовогоУчета,
	|	ВЫБОР КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход
	...
	|	0 КАК СтоимостьЗабалансовая,
	|	0 КАК СтоимостьЗабалансоваяРегл,
	|	СУММА(ВЫБОР КОГДА Партии.СлужебноеВидДвиженияПриход ТОГДА -1 ИНАЧЕ 1 КОНЕЦ * Партии.НДСРегл) КАК СтоимостьРегл,
	|	СУММА(ВЫБОР КОГДА Партии.СлужебноеВидДвиженияПриход ТОГДА -1 ИНАЧЕ 1 КОНЕЦ * Партии.НДСУпр) КАК СтоимостьУпр,
	|	0 КАК ДопРасходыРегл,
	|	0 КАК ДопРасходыУпр,
	|	0 КАК НДСРегл,
	|	0 КАК НДСУпр,
...
// - ВТИсключитьКорректировки - таблица документов "Корректировка приобретения", которые исключаются из учета НДС.
//		Формируется по движениям регистра "Себестоимость товаров" с отбором по временной таблице ВТКорректировкиВДокументеПоступления.
//		Вид корректировки должен быть КорректировкаПоСогласованиюСторон.
//		Корректировка должна быть только на уменьшение стоимости.
...
// - ВТСтоимостьПартийНДС - итоговая таблица детализации партий. Формируется на основании временной таблицы ВТСтоимостьПартийНДСПредварительная.
//		Поля СтатьяСписанияНДС и АналитикаСписанияНДС заполняются по данным временной таблицы ВТДокументыСписанияНДС.
//		Из таблицы исключатся корректировки приобретения, не учитыаемые для целей НДС, из временной таблицы ВТИсключитьКорректировки.
//
//
// Возвращаемое значение:
...
Функция ТекстЗапросаДетализацияПартийНДС(ТекстОрганизация = "", ТекстНачалоПериода = "", ТекстКонецПериода = "", СтруктураОтбора = Неопределено, СтруктураДанных = Неопределено) Экспорт
	...
	|			ИНАЧЕ ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности,
	|		ЕСТЬNULL(Т.КорНаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)))
	|	КОНЕЦ КАК КорНаправлениеДеятельности,
	|   Т.КорГруппаПродукции,
	|
	|	ЕСТЬNULL(РаспределениеЗатратПоИдентификатору.ДокументПоступленияРасходов, Цены.ДокументПоступления) КАК ДокументПоступления,
	|	ЕСТЬNULL(РаспределениеЗатратПоИдентификатору.АналитикаУчетаПартий, Цены.АналитикаУчетаПартийДокументаПоступления) КАК АналитикаУчетаПартийДокументаПоступления,
	...
	|	И (Цены.Организация ЕСТЬ NULL ИЛИ Цены.Период <= Т.Период)
	|	И ВЫБОР
	|	КОГДА &ОтражениеВРеглУчете ТОГДА
	|		ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.РаспределениеПрочихЗатрат)
